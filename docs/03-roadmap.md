...
**当前 schema**：v38 —— 已落地 `player.homeNodeId` / `player.housing.tierId` / `player.job` / `player.shop`；体力、租金、工资、升级费用和累计营业天数仍由普通 `stats` / `flags` 持有，住所等级与升级路径由 `world.economy` 数据配置。常驻软目标是从这些事实实时派生，因此本切片不需要 v39。
...
**当前开发状态（实际 schema v38）**：已完成“移动端后台恢复优先”首个切片，以及设置页“可选外部向量记忆 API”补全切片。后者默认关闭，配置/密钥与派生索引只保存在浏览器 IndexedDB；面对面回复至多增加一次批量 embedding 请求，失败回退本地检索，不需要 v39。终端 UI、联系人/消息/通话、夜间来信、远程约定、音频保活、音乐 App 与 PWA 适配仍未开始。
