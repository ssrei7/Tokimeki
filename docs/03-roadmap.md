...
| 地图空白处点击新建地点 | 属于阶段 3 的地图编辑能力 | 仅在明确的编辑模式中开放；由确定性代码创建节点与连接边，名称、区域、开放时段等需用户确认；可按用户要求调用一次 `map_gen` 生成可继续编辑的名称与描述草稿，但 AI 不得直接写入状态 |
...
4. 地图编辑模式下点击空白位置可创建单个地点，并显式选择连接锚点；退出编辑模式后，普通点击不得误创建地点。用户可选择输入要求并调用一次 `map_gen` 填写名称与描述草稿，生成结果可继续手动编辑，只有确认保存后才写入确定性状态。
