...
**阶段 3 后续增强（已确认范围，不阻塞基础阶段验收）**

- `map_gen` 增加用户要求输入：世界风格、地点类型、规模、布局偏好和必含地点；文本只作为生成建议，仍必须经过 schema、引用和连通性校验。
- 增加局部地图扩展模式：用户指定一个锚点并生成单个或多个新地点，保留已有节点 ID、坐标、路线和玩家位置；建议复用 `map_gen` 任务的显式 `expand` 模式，不新增 API 任务。
- graph / hotspot 地图增加纯本地缩放、平移、回到当前位置等地图 App 交互；不得因此增加 API 调用。
