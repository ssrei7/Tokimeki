...
**当前 schema**：v40 —— 已落地 `player.homeNodeId` / `player.housing.tierId` / `player.job` / `player.shop`；体力、租金、工资、升级费用和累计营业天数仍由普通 `stats` / `flags` 持有，住所等级与升级路径由 `world.economy` 数据配置。阶段 9 首个联系人切片新增 `world.terminal` 容器，并由 v38→v39 migration 初始化；TTS 切片由 v39→v40 为语音消息补充可选音频元数据和请求去重标识。
...
**当前开发状态（实际 schema v40）**：已完成“移动端后台恢复优先”与设置页“可选外部向量记忆 API”补全切片；后者默认关闭，配置/密钥与派生索引只保存在浏览器 IndexedDB。阶段 9 已完成联系人候选、头像/地点本地查询、好友申请状态、v38→v39 migration，终端文本消息、同线程引用、本地/外链贴图、显式生成回复，双向转账（玩家转账即时扣款、好友转入待确认），显式 TTS 语音消息与 v39→v40 migration，纯本地通话壳和通话记录，以及夜间主动来信。远程约定、音频保活、音乐 App 与 PWA 适配仍未开始。
