...
**状态（2026-09-11）**：阶段 8 已完成前四个垂直切片。v34 交付“通用货币配置 + 租房签约 + 到期扣租闭环”；v35 交付“确定性岗位 + 节点绑定 + 班次占用 + 工资结算”；v36 交付“普通 stat 体力 + 成本表扣除 + 休息恢复 + 设置开关”；v37 交付“店铺经营权 + 营业时段 + 累计营业天数 + 日程驱动的反向偶遇”。四个切片均复用既有页面，不新增页面入口。
...
**当前 schema**：v37 —— 已落地 `player.homeNodeId` / `player.housing` / `player.job` / `player.shop`；体力、租金、工资和累计营业天数仍由普通 `stats` / `flags` 持有。
