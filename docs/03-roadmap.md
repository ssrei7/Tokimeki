...
**当前开发状态（实际 schema v40）**：已完成“移动端后台恢复优先”与设置页“可选外部向量记忆 API”补全切片；后者默认关闭，配置/密钥与派生索引只保存在浏览器 IndexedDB。阶段 9 已完成联系人候选、头像/地点本地查询、好友申请状态、v38→v39 migration，终端文本消息、同线程引用、本地/外链贴图、显式生成回复，双向转账（玩家转账即时扣款、好友转入待确认），以及显式 TTS 语音消息与 v39→v40 migration。通话、夜间来信、远程约定、音频保活、音乐 App 与 PWA 适配仍未开始。
