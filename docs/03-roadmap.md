...
**状态（2026-09-11）**：阶段 8 已完成前两个垂直切片。v34 交付“通用货币配置 + 租房签约 + 到期扣租闭环”；v35 交付“确定性岗位 + 节点绑定 + 班次占用 + 工资结算”。两者均复用日程页、晨报与最近结算，不新增页面入口。
...
**当前 schema**：v35 —— 已落地 `player.homeNodeId` / `player.housing` / `player.job`；`player.shop` 留待后续开店切片。
...
3. `energyCost`：普通 stat 扣除、休息恢复与可关闭开关；不为体力新增专用数值字段。
