...
编辑器可为当前页面配置 Agent 运行预算：最大步骤数、最大 API 请求数、单次输出 token、总输出 token 和上下文安全余量。请求前按“中文字符 × 0.7、其他字符 ÷ 4”近似估算完整输入；若超过 `Provider contextWindow - 有效输出上限 - safetyMargin`，会在联网前拒绝。有效输出上限取 Provider 配置、单次上限和剩余总上限的最小值，并实际覆盖本次适配器参数。当前每次发送仍只执行第 1 步和 1 次 API；更大的步骤/请求上限是后续有界修复循环的硬边界，尚不会触发额外调用。预算不持久化，界面只显示近似 token 与请求用量，不估算货币价格。
