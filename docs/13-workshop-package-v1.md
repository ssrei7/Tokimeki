...
本文记录创意工坊首个垂直切片的公开协议和安全边界。根目录 `AGENTS.md` 始终优先。

## 1. 当前能力

当前版本只负责在浏览器本地读取、校验、预览、安装、按世界启停、导出和卸载工坊包。安装后的页面、规则、事件、Prompt 和 Provider 动作不会运行，也不会出现在终端桌面；运行时属于后续切片。
...
v1 安装器会保存这些声明，但不会执行。任意代码、DOM 访问、自定义 CSS、动态 import、脚本 URL 和任意网络请求均没有协议入口。
...
- 包定义全局保存于 Content IndexedDB。
- 启用状态以 `saveId + packageId` 按世界隔离。
- 图片二进制保存于 Assets IndexedDB，包记录只保存 `AssetRef`。
- SaveFile 保持 v41，不保存工坊包、Provider 或本地编辑状态。

停用只改变当前世界绑定。全局卸载会显示受影响的世界数量并移除全部绑定；首版不会自动删除图片二进制，以免在缺少完整引用证明时误删用户资源。工坊资产引用已加入通用完整性检查和保守图片清理引用根。
