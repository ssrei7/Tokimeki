...
代码侧全生命周期验收使用仅存在于测试中的临时依赖包和 Agent 工程，不向产品内置或发布固定官方示例。验收通过 Mock Provider 驱动真实 Agent 工具协议，并在 fake-indexeddb 上运行实际 Dexie 数据库，覆盖创建、ZIP 安装、依赖拓扑、活动、事件、显式 Provider 文本、patch 更新、状态保留、分享导出、卸载、重新导入、完整性审计和全局备份。测试不调用真实 API，也不写用户浏览器数据。

真实 Provider 的模型输出质量、兼容端点和实际界面体验仍需用户显式手测；它们不改变本协议的本地校验、权限、预算和状态所有权边界。
