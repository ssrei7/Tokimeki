...
`DirectorPreferences` 属于当前世界存档的叙事偏好，不进入世界包。完整导演上下文注入 `narrate_main` 与 `topic_tree`；统一 NPC 偏好投影还可注入现有 `map_activity_gen` 和活动新人候选请求，但不进入摘要、状态抽取或晨报移动等无关结构化任务。`npcPreferenceTags` / `avoidTags` 还会以有界、确定性的本地因子调整半正式 NPC 相遇权重，正式角色不受影响。偏好不直接修改关系、金钱、时间、地点、日程或已有角色资料；v43→v44 migration 会为旧存档补齐空偏好。
...
地图右上角“地点动态”抽屉继续作为全图管理入口；点击地图锚点后打开的地点详情也提供同一份 `world.placeHighlights` 的当前地点筛选、单地点生成、手工新建、编辑和删除入口。两处入口共用内核 CRUD 与草稿确认流程，不创建第二份地点动态数据，也不改变 v43 schema。
