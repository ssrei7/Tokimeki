...
export class PromptAssembler {
  ...
  listBlocks(): PromptBlock[] { return [...this.blocks.values()]; }
  assemble(facts: PromptFacts, options: { budget: number; task?: string }): AssembledPrompt {
    ...
    return { messages: results.filter((result) => !result.dropped && !result.skipped).map((result) => ({ role: result.role, content: result.text })), blocks: results, estimatedTokens: results.reduce((sum, result) => sum + result.estimatedTokens, 0), budget: options.budget };
  }
}
