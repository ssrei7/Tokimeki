...
export function createDefaultPromptBlocks(opPromptDocs = ''): PromptBlock[] {
  ...
  return [
    { id: 'format_contract', role: 'system', priority: 100, order: 0, build: (facts) => {
      ...
      return `你是开放世界叙事游戏中的角色。先输出自然语言正文。面对面场景中只有明确写成 [说话人:角色名] 的内容才是角色台词；环境、动作、心理或其他描写一律使用 [旁白] 内容。未标记的助手正文为兼容旧记录，界面会按旁白显示。游戏状态只由确定性内核持有，不要声称提议已经生效。${presetText}${opContract}`;
    } },
