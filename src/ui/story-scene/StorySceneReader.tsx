...
export function StorySceneReader({ scene, onAdvanceStage, onSetStatus, onReadStage, onSelectStage }: StorySceneReaderProps) {
  ...
  return (
    <article className="story-reader" aria-label={`剧情阅读：${scene.title}`}>
      ...
      <div className="story-reader-footer">{nextUnread ? <p className="story-reader-hint">下一阶段：{nextUnread.title}。剧情推进后才会解锁更远内容。</p> : <p className="story-reader-hint">当前已解锁内容均已读完。</p>}<div className="button-row">{onAdvanceStage && scene.status === 'active' && scene.stages[scene.stages.length - 1]?.id !== scene.currentStageId && <button type="button" onClick={onAdvanceStage}>推进下一阶段</button>}{onSetStatus && scene.status === 'active' && <><button type="button" className="secondary" onClick={() => onSetStatus('completed')}>标记完成</button><button type="button" className="danger" onClick={() => onSetStatus('cancelled')}>取消剧情</button></>}</div></div>
    </article>
  );
}
