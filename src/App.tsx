...
import { advanceStorySceneStage, confirmStoryScene, createBuiltinStoryScenePresets, createStorySceneDraft, deleteStorySceneDraft, formatChatArchive, formatEventHistoryArchive, generateStorySceneDraftInput, readStorySceneStage, selectStorySceneReadingStage, updateStorySceneDraft, updateStorySceneStatus, type StorySceneDraftInput } from './core/story';
...
export function App() {
  ...
  return <div className="app-shell">
    ...
    <main className={`screen ${tab === 'chat' ? 'chat-screen-host' : ''} ${tab === 'map' ? 'map-screen-host' : ''}`}>
      ...
      {tab === 'library' && <StorySceneLibraryView save={save} onCreateDraft={createStorySceneDraftFromInput} onEditDraft={editStorySceneDraft} onDeleteDraft={removeStorySceneDraft} onConfirmDraft={confirmStorySceneDraft} onAdvanceStage={advanceStoryScene} onSetStatus={setStorySceneStatus} onReadStage={(sceneId, stageId) => updateStorySceneReading(sceneId, stageId, 'read')} onSelectStage={(sceneId, stageId) => updateStorySceneReading(sceneId, stageId, 'select')} />}
