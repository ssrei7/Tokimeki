...
import { exportCharacterPackage, exportPresetBundle, exportSaveZip, exportWorldPackage, importCharacterPackage, importPresetBundle, importSaveZip, importWorldPackage } from './data/io/zip';
...
import { importPlainText, type TextImportKind } from './data/text-import';
...
export function App() {
  ...
  async function importTextContent(kind: TextImportKind, file?: File): Promise<void> {
    if (!file) return;
    try {
      const imported = importPlainText(kind, file.name, await file.text(), now());
      ...
      setFeedback({ tone: 'success', text: `TXT 已导入为${kind === 'character' ? '角色卡' : kind === 'worldbook' ? '世界书' : '预设'}，请继续编辑确认。` });
    } catch (error) { setFeedback({ tone: 'error', text: errorMessage(error, 'TXT 导入失败。') }); }
  }
