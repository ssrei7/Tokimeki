...
import { collectTokimekiPreferences, exportGlobalBackup, importGlobalBackup, restoreTokimekiPreferences, type GlobalBackupRestoreSelection, type ImportedGlobalBackup } from './data/io/global-backup';
...
import { CharacterProviderBindingSchema, EmbeddingConfigSchema, ImageConfigSchema, ImageVisualConfigSchema, ProviderBindingSchema, ProviderConfigSchema, ProviderSettingSchema, TASK_IDS, TtsConfigSchema, type CharacterProviderBinding, type EmbeddingConfig, type ImageConfig, type ImageVisualConfig, type ProviderBinding, type ProviderConfig, type TaskId, type TtsConfig } from './providers/types';
...
export function App() {
  ...
  useEffect(() => {
    void Promise.all([contentDb.characters.toArray(), contentDb.personas.toArray(), contentDb.worldbooks.toArray(), contentDb.presets.toArray(), contentDb.presetBundles.toArray(), contentDb.storyScenePresets.toArray(), providerDb.providers.toArray(), providerDb.bindings.toArray(), providerDb.characterBindings.toArray(), providerDb.settings.get('defaultProviderId'), providerDb.settings.get('defaultTtsProviderId'), providerDb.embeddingConfigs.get('embedding'), providerDb.ttsConfigs.toArray(), providerDb.imageConfigs.get('image'), providerDb.imageVisualConfigs.toArray(), loadCurrentSave(), listSnapshots()]).then(([c, masks, w, p, bundles, scenePresets, ps, bs, storedCharacterBindings, setting, ttsSetting, storedEmbedding, storedTtsConfigs, storedImageConfig, storedImageVisualConfigs, persistedSave, savedSnapshots]) => {
      ...
      const currentSaveId = persistedSave?.meta.id ?? defaultSave.meta.id;
      setImageVisualConfigs(storedImageVisualConfigs.flatMap((item) => { const parsed = ImageVisualConfigSchema.safeParse(item); return parsed.success && parsed.data.saveId === currentSaveId ? [parsed.data] : []; }));
  ...
  async function saveImageVisualConfig(characterId: string, appearancePrompt: string): Promise<void> {
    const record = ImageVisualConfigSchema.parse({ id: `${saveRef.current.meta.id}:${characterId}`, saveId: saveRef.current.meta.id, characterId, appearancePrompt, updatedAt: now() });
  ...
  async function generateCharacterImage(characterId: string, target: 'avatar' | 'portrait', scenePrompt: string): Promise<void> {
    ...
    let generatedAssetId: string | undefined;
    try {
      const result = await generateImage(provider, prompt, { size: config.size, quality: config.quality, style: config.style, responseFormat: config.responseFormat, referenceMode: 'none' });
  ...
  async function downloadSave() {
    ...
    const assetMeta: Record<string, { mimeType: string; width?: number; height?: number; category?: 'voice'; cacheFingerprint?: string; audioFormat?: string; durationMs?: number; voiceRequestId?: string }> = {};
  ...
  async function downloadProviderSettings(selection: ProviderSettingsSelection): Promise<void> {
    try {
      const pack = createProviderSettingsPackage({ providers, ttsConfigs, bindings, characterBindings, saveId: saveRef.current.meta.id, defaultProviderId, defaultTtsProviderId: defaultTtsConfigId }, selection);
      ...
      setFeedback({ tone: 'success', text: `设置迁移包已导出：${pack.providers.length} 个普通 Provider、${pack.ttsConfigs.length} 个语音 Provider。API key 未导出。` });
  ...
  async function applyProviderSettings(pack: ProviderSettingsPackage, selection: ProviderSettingsSelection): Promise<void> {
    try {
      ...
      const merged = mergeProviderSettingsPackage({ providers, ttsConfigs, bindings, characterBindings, saveId: saveRef.current.meta.id }, selected);
      await providerDb.transaction('rw', providerDb.providers, providerDb.ttsConfigs, providerDb.bindings, providerDb.characterBindings, providerDb.settings, async () => {
  ...
  async function downloadGlobalBackup(includeSecrets: boolean): Promise<void> {
    try {
      ...
      const [currentSave, snapshots, characters, personas, worldbooks, presets, presetBundles, storyScenePresets, chats, chatRecovery, memoryVectors, musicStates, terminalStickers, ps, tts, bs, cbs, settings] = await Promise.all([loadCurrentSave(), listSnapshots(), contentDb.characters.toArray(), contentDb.personas.toArray(), contentDb.worldbooks.toArray(), contentDb.presets.toArray(), contentDb.presetBundles.toArray(), contentDb.storyScenePresets.toArray(), contentDb.chats.toArray(), contentDb.chatRecovery.toArray(), contentDb.memoryVectors.toArray(), contentDb.musicStates.toArray(), contentDb.terminalStickers.toArray(), providerDb.providers.toArray(), providerDb.ttsConfigs.toArray(), providerDb.bindings.toArray(), providerDb.characterBindings.toArray(), providerDb.settings.toArray()]);
      const localStorage = collectTokimekiPreferences(window.localStorage);
      const blob = await exportGlobalBackup({ currentSave, snapshots, content: { characters, personas, worldbooks, presets, presetBundles, storyScenePresets, chats, chatRecovery, memoryVectors, musicStates, terminalStickers }, providers: ps, ttsConfigs: tts, bindings: bs, characterBindings: cbs, settings, localStorage }, assets, includeSecrets);
  ...
  async function previewGlobalBackup(file?: File): Promise<void> {
    ...
    try {
      ...
      setFeedback({ tone: 'info', text: `已读取全局备份：${backup.data.providers.length} 个普通 Provider、${backup.data.ttsConfigs.length} 个语音 Provider、${backup.assets.size} 个资产${backup.hasSecrets ? '，包含 API key' : ''}。尚未写入本地数据。` });
  ...
  async function restoreGlobalBackup(backup: ImportedGlobalBackup, selection: GlobalBackupRestoreSelection = { world: true, content: true, providers: true, assets: true, preferences: true }): Promise<void> {
    ...
    const previous = {
      ...
      providers: selection.providers ? { providers: await providerDb.providers.toArray(), ttsConfigs: await providerDb.ttsConfigs.toArray(), bindings: await providerDb.bindings.toArray(), characterBindings: await providerDb.characterBindings.toArray(), settings: await providerDb.settings.toArray() } : undefined,
    ...
    try {
      ...
      if (selection.preferences) restoreTokimekiPreferences(window.localStorage, backup.data.localStorage);
  ...
  async function checkAssetIntegrity(): Promise<void> {
    ...
    try {
      const [assets, characterCards, chats, savedSnapshots, stickers] = await Promise.all([listAssets(), contentDb.characters.toArray(), contentDb.chats.toArray(), listSnapshots(), listTerminalStickers()]);
  ...
  async function loadSave(file?: File) {
    ...
    try {
      ...
      for (const [id, bytes] of imported.assets) {
        ...
        const copy = new ArrayBuffer(bytes.byteLength); new Uint8Array(copy).set(bytes);
        await saveAsset({ id, blob: new Blob([copy], { type: mimeType }), mimeType, width: typeof metadata?.width === 'number' ? metadata.width : undefined, height: typeof metadata?.height === 'number' ? metadata.height : undefined, category: metadata?.category === 'voice' ? 'voice' : undefined, cacheFingerprint: typeof metadata?.cacheFingerprint === 'string' ? metadata.cacheFingerprint : undefined, audioFormat: typeof metadata?.audioFormat === 'string' ? metadata.audioFormat : undefined, durationMs: typeof metadata?.durationMs === 'number' ? metadata.durationMs : undefined, voiceRequestId: typeof metadata?.voiceRequestId === 'string' ? metadata.voiceRequestId : undefined, createdAt: now() });
      }
...
function ImageSettingsView(props: { config: ImageConfig; providers: ProviderConfig[]; setProvider: (provider: ProviderConfig) => void; busy: boolean; onSave: (config: ImageConfig) => Promise<void>; onTest: (config: ImageConfig) => Promise<void>; onNewProvider: () => void; onOpenProvider: () => void; save: SaveFile; imageVisualConfigs: ImageVisualConfig[]; visualCharacterId: string; setVisualCharacterId: (value: string) => void; imagePrompt: string; setImagePrompt: (value: string) => void; imageTarget: 'avatar' | 'portrait'; setImageTarget: (value: 'avatar' | 'portrait') => void; onSaveImageVisualConfig: (characterId: string, appearancePrompt: string) => Promise<void>; onGenerateCharacterImage: (characterId: string, target: 'avatar' | 'portrait', scenePrompt: string) => Promise<void> }) {
...
function SettingsView(props: {
  ...
  if (props.activePage === 'image') return <SubpageShell title={pageTitle} pageId="image" onBack={props.onBack}><ImageSettingsView config={props.imageConfig} providers={props.providers} setProvider={props.setProvider} busy={props.imageBusy} onSave={props.onSaveImage} onTest={props.onTestImage} onNewProvider={props.onNewProvider} onOpenProvider={props.onOpenProvider} save={props.save} imageVisualConfigs={props.imageVisualConfigs} visualCharacterId={props.visualCharacterId} setVisualCharacterId={props.setVisualCharacterId} imagePrompt={props.imagePrompt} setImagePrompt={props.setImagePrompt} imageTarget={props.imageTarget} setImageTarget={props.setImageTarget} onSaveImageVisualConfig={props.onSaveImageVisualConfig} onGenerateCharacterImage={props.onGenerateCharacterImage} /></SubpageShell>;
