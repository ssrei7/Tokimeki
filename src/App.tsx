...
import { createEconomyOpRegistry, formatCurrency, getRentalQuote, injectEconomyMorningAds, registerEconomyHooks } from './features/economy';
...
const defaultSave: SaveFile = SaveFileSchema.parse({
  ...
  world: {
    ...
    slotsUsedToday: 0,
    player: { name: '旅人', nodeId: 'start', stats: { 'custom-reputation': 0, money: 0, 'economy.rent.amount': 10, 'economy.rent.interval-days': 7 }, flags: {}, inventory: [] },
    stats: {}, flags: {},
...
function DayView(props: { save: SaveFile; snapshots: SaveSnapshot[]; morningBriefs: SaveFile['world']['morningBriefs']; morningUpdates: SaveFile['world']['morningUpdates']; morningStyle: SaveFile['config']['morningStyle']; summarizingDay: number | null; onAction: (kind: string) => void; onAcceptRental: (nodeId: string, rentRuleId?: string) => void; onSleep: () => void; onRestoreSnapshot: (id: string) => Promise<void>; onSaveDiary: (day: number, text: string) => void; onPresetChange: (preset: SaveFile['config']['calendar']['preset']) => void; onRevealEvent: (scheduledId: string) => void; onResolveEventChoice: (historyId: string, choiceId: string) => void; onExportEventHistory: () => void; onExportChatArchive: () => Promise<void>; canExportChatArchive: boolean; onDeleteEventHistory: (id?: string) => void; onMove: (nodeId: string) => void }) {
  ...
  return <section>
    ...
    <div className="day-card"><label>每日节奏<select value={calendar.preset} disabled={used > 0} onChange={(event) => props.onPresetChange(event.target.value as SaveFile['config']['calendar']['preset'])}><option value="leisure">悠闲 · 6 时段</option><option value="standard">标准 · 4 时段</option><option value="tight">紧凑 · 3 时段</option><option value="sandbox">沙盒 · 不消耗</option></select></label><p className="io-scope">行动只修改本地确定性状态，不调用 API。节奏仅能在当天尚未行动时切换。</p>{housing && <p className="io-scope">住所：{housingNode?.name ?? housing.nodeId}{Number.isFinite(nextRentDay) ? ` · 下次租金第 ${nextRentDay} 天结算` : ''}</p>}<div className="day-actions">{actionButtons.map((action) => <button key={action.kind} onClick={() => props.onAction(action.kind)} disabled={!calendar.unlimitedSlots && action.cost > remaining}>{action.label}<small>{calendar.unlimitedSlots ? '不消耗' : `${action.cost} 时段`}</small></button>)}<button className="secondary" onClick={props.onSleep}>推进世界<small>结算今天 · 进入下一天</small></button></div></div>
