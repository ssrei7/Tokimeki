import { createContext, useContext, useEffect, useLayoutEffect, useMemo, useRef, useState, type CSSProperties, type MouseEvent, type PointerEvent, type WheelEvent } from 'react';
...
import { buildTerminalReplyPrompt, confirmTerminalAppointment, createFriendRequest, createIncomingTransferProposal, createTerminalAppointmentRequest, deliverNightlyTerminalMessage, isAcceptedFriend, listContactCandidates, listTerminalAppointmentRequests, listTerminalCalls, listTerminalMessages, listTerminalTransfers, recordTerminalCall, resolveFriendRequest, resolveIncomingTransfer, resolveTerminalAppointmentRequest, sendPlayerTransfer, sendTerminalReplyMessage, sendTerminalStickerMessage, sendTerminalTextMessage, sendTerminalVoiceMessage, simulateFriendAcceptance, simulateTerminalAppointmentAcceptance, type ContactDirection, type TerminalAppointmentAction, type TerminalAppointmentInput, type TerminalCallStatus, type TransferAction } from './core/terminal';
...
export const DAY_PAGE_DEFINITIONS: readonly (DesktopEntry & { id: DayPage; pageTitle: string })[] = [
  ...
  { id: 'story', label: '剧情', pageTitle: '剧情进展', icon: Milestone, tone: 'gray' },
...
export function App() {
  ...
  function requestTerminalFriend(characterId: string, direction: ContactDirection): void {
    ...
    setFeedback({ tone: 'success', text: result.changed ? '好友申请已保存。' : '已有相同状态的好友申请。' });
  }
  ...
  function simulateTerminalFriendAcceptance(requestId: string): void {
    const next = structuredClone(saveRef.current);
    const result = simulateFriendAcceptance(next.world, requestId);
    if (!result.ok) { setFeedback({ tone: 'error', text: result.warning ?? '模拟好友确认失败。' }); return; }
    if (result.changed) commitSave(next);
    setFeedback({ tone: 'success', text: result.changed ? '已模拟对方接受好友申请。' : '该好友申请已经确认。' });
  }
  ...
  function createIncomingTerminalTransfer(characterId: string, currencyId: string, amount: number): void {
    const next = structuredClone(saveRef.current);
    const result = createIncomingTransferProposal(next.world, characterId, currencyId, amount);
    if (!result.ok) { setFeedback({ tone: 'error', text: result.warning ?? '转入提议创建失败。' }); return; }
    commitSave(next);
    setFeedback({ tone: 'success', text: '已模拟对方发来转账提议，等待你确认。' });
  }

  function resolveIncomingTerminalTransfer(requestId: string, action: TransferAction): void {
    const next = structuredClone(saveRef.current);
    const result = resolveIncomingTransfer(next.world, requestId, action);
  ...
  async function generateTerminalReply(characterId: string): Promise<void> {
    ...
    const request = current.world.terminal.friendRequests.find((item) => item.characterId === characterId && item.status === 'accepted');
    const character = current.world.characters[characterId] ?? current.world.npcs[characterId];
    if (!request || !character) { setFeedback({ tone: 'error', text: '只有已接受的好友可以生成终端回复。' }); return; }
    ...
    let narrative = '';
    try {
      await streamChat(parsed, prompt, (delta) => { narrative += delta; }, { taskId: 'narrate_main' });
      const next = structuredClone(saveRef.current);
      const result = sendTerminalReplyMessage(next.world, characterId, narrative);
...
function DayView(props: { activePage: DayPage | null; onOpenPage: (page: DayPage) => void; onBack: () => void; save: SaveFile; snapshots: SaveSnapshot[]; morningBriefs: SaveFile['world']['morningBriefs']; morningUpdates: SaveFile['world']['morningUpdates']; morningStyle: SaveFile['config']['morningStyle']; summarizingDay: number | null; onAction: (kind: string) => void; onAcceptRental: (nodeId: string, rentRuleId?: string) => void; onRequestHousingUpgrade: (upgradeRuleId: string) => void; onAcceptJob: (nodeId: string, jobRuleId?: string) => void; onWorkJob: () => void; onAcceptShop: (nodeId: string, shopRuleId?: string) => void; onOperateShop: () => void; onSleep: () => void; onRestoreSnapshot: (id: string) => Promise<void>; onSaveDiary: (day: number, text: string) => void; onPresetChange: (preset: SaveFile['config']['calendar']['preset']) => void; onRevealEvent: (scheduledId: string) => void; onResolveEventChoice: (historyId: string, choiceId: string) => void; onExportEventHistory: () => void; onExportChatArchive: () => Promise<void>; canExportChatArchive: boolean; onDeleteEventHistory: (id?: string) => void; onMove: (nodeId: string) => void }) {
  ...
  const dayEntries = DAY_PAGE_DEFINITIONS.map((entry) => {
    if (entry.id === 'housing' && housing) return { ...entry, badge: housingTier?.name };
    if (entry.id === 'career' && (job || shop)) return { ...entry, badge: [job ? '工作' : '', shop ? '店铺' : ''].filter(Boolean).join('·') };
    if (entry.id === 'events' && props.save.world.eventHistory.length) return { ...entry, badge: String(props.save.world.eventHistory.length) };
    return entry;
  });
  ...
  if (props.activePage) return <SubpageShell eyebrow="日程" title={DAY_PAGE_DEFINITIONS.find((entry) => entry.id === props.activePage)?.pageTitle ?? '日程'} pageId={props.activePage} onBack={props.onBack}>{renderDaySubpage(props.activePage)}</SubpageShell>;
  return <section>
    <DesktopLauncher title="日程" entries={dayEntries} onOpen={(id) => props.onOpenPage(id as DayPage)} />
...
function ContactsView(props: { save: SaveFile; onRequestFriend: (characterId: string, direction: ContactDirection) => void; onResolveFriend: (requestId: string, action: 'accept' | 'reject' | 'revoke') => void; onSimulateFriendAcceptance: (requestId: string) => void }) {
  ...
  const card = (candidate: (typeof candidates)[number]) => <article className="contact-card" key={candidate.id} data-character-id={candidate.id}><ContactAvatar name={candidate.name} avatar={candidate.avatar} /><div className="contact-card-main"><strong>{candidate.name}</strong><span>{candidate.tier === 'formal' ? '正式角色' : '半正式角色 / NPC'}</span><small>{locationText(candidate)}</small></div><div className="contact-card-actions">{!candidate.request && <><button type="button" onClick={() => props.onRequestFriend(candidate.id, 'outgoing')}>我加TA</button><button type="button" className="secondary" onClick={() => props.onRequestFriend(candidate.id, 'incoming')}>TA加我</button></>}{candidate.request?.status === 'pending' && candidate.request.direction === 'outgoing' && <><span className="contact-status">已发出申请</span><button type="button" onClick={() => props.onSimulateFriendAcceptance(candidate.request!.id)}>模拟TA接受</button><button type="button" className="secondary" onClick={() => props.onResolveFriend(candidate.request!.id, 'revoke')}>撤回</button></>}{candidate.request?.status === 'pending' && candidate.request.direction === 'incoming' && <><button type="button" onClick={() => props.onResolveFriend(candidate.request!.id, 'accept')}>接受</button><button type="button" className="secondary" onClick={() => props.onResolveFriend(candidate.request!.id, 'reject')}>拒绝</button></>}{candidate.request?.status === 'accepted' && <span className="contact-status">已接受</span>}</div></article>;
  ...
  return <div className="contacts-view" data-testid="terminal-contacts">{section('待加好友', available, '暂无可添加的联系人。')}{section('待处理申请', pendingIncoming, '暂无待处理申请。')}{section('已接受好友', accepted, '还没有已接受的好友。')}{section('历史申请', history, '暂无历史申请。')}</div>;
}
  ...
  return <div className="terminal-messages-view" data-testid="terminal-messages">
    ...
    <section className="terminal-transfer-panel" aria-label="转账">
      ...
      <div className="terminal-transfer-form">
        ...
        <button type="button" className="secondary" onClick={() => props.onCreateIncomingTransfer(selected!.id, currencyId, Number(transferAmount))} disabled={!currency || !transferAmount.trim()}>模拟TA转入</button>
      </div>
