...
function DayView(props: { save: SaveFile; snapshots: SaveSnapshot[]; morningBriefs: SaveFile['world']['morningBriefs']; morningUpdates: SaveFile['world']['morningUpdates']; summarizingDay: number | null; onAction: (kind: string) => void; onSleep: () => void; onRestoreSnapshot: (id: string) => Promise<void>; onSaveDiary: (day: number, text: string) => void; onPresetChange: (preset: SaveFile['config']['calendar']['preset']) => void; onMove: (nodeId: string) => void }) {
  ...
  return <section>
    ...
    <div className="morning-brief-card"><div className="list-heading"><h3>今日晨报</h3><span className="io-scope">最多每日一次合并更新</span></div>{todayUpdate && <div className="morning-world-meta"><span>天气：{todayUpdate.weather.label}</span>{todayUpdate.worldNote && <span>{todayUpdate.worldNote}</span>}</div>}{todayBriefs.length ? <div className="morning-brief-list">{todayBriefs.map((entry) => { const destination = resolveMorningAdDestination(entry, props.save.world); const isJobAd = entry.category === 'ad' && entry.entryKind === 'job'; return <article key={entry.id} className={`morning-brief-entry morning-${entry.category}`}><span className="eyebrow">{entry.category}</span><strong>{entry.title}</strong><p>{entry.body}</p>{(destination || isJobAd) && <div className="morning-entry-actions">{destination && <button className="secondary" onClick={() => props.onMove(destination.id)}>前往{destination.name}</button>}{isJobAd && <button onClick={() => setActiveAd(entry)}>查看招聘</button>}</div>}</article>; })}</div> : <p className="empty">今天还没有晨报；进入新一天后会生成。</p>}{activeAd?.entryKind === 'job' && <div className="morning-ad-entry"><div className="list-heading"><h3>招聘入口</h3><button className="secondary" onClick={() => setActiveAd(null)}>关闭</button></div><p>{activeAd.body}</p><button onClick={() => { props.onAction('work'); setActiveAd(null); }} disabled={!calendar.unlimitedSlots && (props.save.config.actionCosts.work?.slotCost ?? 0) > remaining}>接受招聘并开始工作<small>{calendar.unlimitedSlots ? '不消耗时段' : `${props.save.config.actionCosts.work?.slotCost ?? 0} 时段`}</small></button></div>}</div>
