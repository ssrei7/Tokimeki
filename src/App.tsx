...
import { createDefaultPromptBlocks, type EncounterPromptParticipant } from './core/prompt/default-blocks';
...
import { createMapNode, deleteMapNode, movePlayer, parseGeneratedMap, parseGeneratedMapExpansion, parseGeneratedNodeSuggestion, revealAdjacentNode, updateMapNode, type CreateMapNodeInput, type UpdateMapNodeInput } from './core/map';
...
import { attachTerminalVoiceToMessage, buildTerminalReplyPrompt, confirmTerminalAppointment, createFriendRequest, createTerminalAppointmentRequest, createTerminalOpRegistry, deleteTerminalMessage, deliverNightlyTerminalMessage, editTerminalMessage, isAcceptedFriend, listContactCandidates, listTerminalAppointmentRequests, listTerminalCalls, listTerminalMessages, listTerminalMessageThreads, listTerminalTransfers, recordTerminalCall, resolveFriendRequest, resolveIncomingTransfer as resolveIncomingTransferOp, resolveTerminalAppointmentRequest, sendPlayerTransfer, sendTerminalRejoinRequest, sendTerminalReplyMessage, sendTerminalStickerMessage, sendTerminalTextMessage, TERMINAL_PLAYER_ID, type ContactDirection, type TerminalAppointmentAction, type TerminalAppointmentInput, type TerminalCallStatus, type TransferAction } from './core/terminal';
...
import { PresetBundleSchema, PresetSchema, normalizeChatMessages, parseWorldbookKeys, type CharacterCard, type ChatCgAttachment, type ChatMessage, type ChatRecord, type ChatRecoveryRecord, type Persona, type Preset, type PresetBundle, type TerminalStickerRecord, type VoiceAttachment, type WorldbookEntry } from './data/content';
...
import { auditWorkshopIntegrity, type WorkshopIntegrityReport } from './data/workshop-integrity';
...
import { createDefaultMap, CURRENT_SCHEMA_VERSION, DEFAULT_ACTION_COSTS, DEFAULT_DIRECTOR_STATE, DEFAULT_ECONOMY_STATE, DEFAULT_SLOT_DEFS, DEFAULT_TERMINAL_STATE, DirectorPreferencesSchema, SaveFileSchema, type AssetRef, type CollectionEntry, type DirectorPreferences, type EncounterDeparture, type GiftHistoryEntry, type PlaceHighlight, type SaveFile, type Topic, type TopicTree } from './data/schema/save';
...
import { generateWorkshopDraft as requestWorkshopDraft, runWorkshopAgentTurn as requestWorkshopAgentTurn, type WorkshopAgentTurnInput } from './providers/workshop-draft';
...
import { prepareWorkshopProviderText, type WorkshopProviderTextAction } from './features/workshop-provider';
import { requestWorkshopProviderText } from './providers/workshop-text';
import type { WorkshopLocalValue, WorkshopPackageRecord } from './data/workshop';
import { AlertTriangle, ArrowLeft, Backpack, BookOpen, Bot, BrainCircuit, Bug, CalendarDays, Camera, Check, ChevronDown, ChevronUp, Clapperboard, ContactRound, Download, FileArchive, Gift, History, House, Image as ImageIcon, LogOut, MessageCircle, Milestone, Music2, NotebookPen, Palette, Phone, PhoneIncoming, PhoneOff, Plus, ReceiptText, RefreshCw, Reply, Route, RotateCcw, Send, ShieldCheck, SlidersHorizontal, Smile, Sparkles, Target, UserRound, UsersRound, BriefcaseBusiness, Wrench, X } from 'lucide-react';
...
import { PlaceNodeHighlights } from './components/place-node-highlights';
import { PlaceActivityDialog } from './components/place-activity-dialog';
import { DirectorPreferencesView } from './components/director-preferences';
...
export type LibraryPage = 'messages' | 'contacts' | 'calls' | 'music' | 'workshop' | 'memories' | 'collection' | 'inventory' | 'characters' | 'worldbook' | 'presets' | 'event-packages' | 'director' | 'story' | 'save' | 'calendar' | 'goals' | 'housing' | 'career' | 'diary' | 'events' | 'progress' | 'snapshots';
export type LibraryRoute = LibraryPage | `workshop-app:${string}`;
...
const LibraryNavigationContext = createContext<{ activePage: LibraryRoute | null; onOpenPage: (page: LibraryRoute) => void; onBack: () => void }>({ activePage: null, onOpenPage: () => undefined, onBack: () => undefined });
export const LIBRARY_PAGE_DEFINITIONS: readonly (DesktopEntry & { id: LibraryPage; pageTitle: string })[] = [
  ...
  { id: 'director', label: '剧情导演', pageTitle: '剧情导演', icon: Clapperboard, tone: 'gray' },
...
const defaultSave: SaveFile = SaveFileSchema.parse({
  ...
  world: {
    ...
    relations: {}, characters: {}, npcs: {}, npcTemplates: {}, encounterLog: [], topicTrees: {}, usedTopics: {}, appointments: [], eventDefs: {}, director: structuredClone(DEFAULT_DIRECTOR_STATE), eventHistory: [], chapters: [], milestones: [], storyScenes: [], collection: [], economy: structuredClone(DEFAULT_ECONOMY_STATE), terminal: structuredClone(DEFAULT_TERMINAL_STATE),
...
export function App() {
  ...
  const [libraryPage, setLibraryPage] = useState<LibraryRoute | null>(null);
  ...
  const [worldbookKeys, setWorldbookKeys] = useState('');
  ...
  const [workshopIntegrityReport, setWorkshopIntegrityReport] = useState<WorkshopIntegrityReport | null>(null);
  ...
  const [topicMode, setTopicMode] = useState<EncounterSceneMode>('manual');
  const [topicLoading, setTopicLoading] = useState(false);
  const [topicRetryContext, setTopicRetryContext] = useState<TopicRetryContext | null>(null);
  const [openingRetryAvailable, setOpeningRetryAvailable] = useState(false);
    ...
    void Promise.all([loadChat(selectedCharacterId), loadChatRecovery(selectedCharacterId)]).then(([record, recovery]) => {
      ...
      if (restored) {
        ...
        if (restored.status === 'interrupted' || restored.status === 'error') {
          ...
          if (restored.raw && !restored.opsApplied && !saveRef.current.world.npcs[selectedCharacterId]) setPendingOps({ raw: restored.raw, actorId: restored.actorId, messageIndex: restored.messageIndex, streamError: restored.error, requestId: restored.requestId });
  ...
  const presentChatCharacters = useMemo<EncounterPromptParticipant[]>(() => {
    const presentIds = whoIsHere(save.world, save.world.player.nodeId, save.world.clock.day, save.world.clock.slotId, save.config.calendar.daysPerWeek).map((person) => person.id);
    return encounterPromptParticipants(save.world, characters, presentIds);
  ...
  const enabledWorkshopPackageRecords = useEnabledWorkshopPackages(save.meta.id);
  const enabledWorkshopPackages = useMemo(() => enabledWorkshopPackageRecords ?? [], [enabledWorkshopPackageRecords]);
  const workshopPackagesLoaded = enabledWorkshopPackageRecords !== undefined;
  ...
  useEffect(() => registerWorkshopActivityHooks(promptEvents, enabledWorkshopPackages), [enabledWorkshopPackages, promptEvents]);

  useEffect(() => {
    if (!workshopPackagesLoaded) return;
    const next = structuredClone(saveRef.current);
    const synced = syncWorkshopEventDefinitions(next, enabledWorkshopPackages);
    if (synced.changed) commitSave(next);
    if (synced.warnings.length) setFeedback({ tone: 'info', text: synced.warnings.join(' ') });
  }, [enabledWorkshopPackages, workshopPackagesLoaded]);
  ...
  function createTerminalAppointment(characterId: string, input: TerminalAppointmentInput, direction: 'outgoing' | 'incoming' = 'outgoing'): void {
    ...
    if (result.changed) commitSave(next);
    setFeedback({ tone: 'success', text: result.changed ? direction === 'outgoing' ? 'TA已接受约定，现在可以加入日历。' : 'TA已发来约定提议。' : '已有相同的约定提议。' });
  }

  function saveDirectorPreferences(preferences: DirectorPreferences): void {
    try {
      const next = structuredClone(saveRef.current);
      next.world.director.preferences = DirectorPreferencesSchema.parse(preferences);
      commitSave(next);
      setFeedback({ tone: 'success', text: '剧情导演设置已保存到当前世界。' });
    } catch (error) {
      setFeedback({ tone: 'error', text: errorMessage(error, '剧情导演设置无效。') });
    }
  }

  function simulateTerminalAppointmentAcceptanceForUi(requestId: string): void {
    const request = saveRef.current.world.terminal.appointmentRequests.find((item) => item.id === requestId);
    setFeedback({ tone: request?.status === 'accepted' ? 'success' : 'error', text: request?.status === 'accepted' ? 'TA已接受约定，现在可以加入日历。' : '该约定暂时不能加入日历。' });
  }
  ...
  function runDayAction(kind: string): void {
    ...
    const revealedNode = kind === 'explore' ? revealAdjacentNode(next.world) : undefined;
    ...
    if (result.settledDays.length) {
      setTab('day');
      const discovery = revealedNode?.toNodeId ? `已发现地点“${next.world.map.nodes[revealedNode.toNodeId]?.name ?? revealedNode.toNodeId}”。` : '';
      setFeedback({ tone: 'success', text: [discovery, `第 ${result.settledDays.at(-1)} 天已结算，已进入下一天。`].filter(Boolean).join(' ') });
      return;
    }
    const discovery = revealedNode?.toNodeId ? `已发现地点“${next.world.map.nodes[revealedNode.toNodeId]?.name ?? revealedNode.toNodeId}”。` : '';
    const actionSummary = result.advanced ? `行动完成，消耗 ${result.advanced} 个时段。` : '当前模式不消耗时段。';
    setFeedback({ tone: 'info', text: [discovery, actionSummary].filter(Boolean).join(' ') });
  }
  ...
  function operateShop(): void {
    ...
    if (visitEntry && visitCandidates.length) {
      ...
      const participantIds = visitCandidates.map((candidate) => candidate.id);
      setEncounterParticipantIds(participantIds);
      setEncounterPrimaryId(participantIds.length === 1 ? participantIds[0] : '');
      setTab('map');
    }
  ...
  function resolveChoice(historyId: string, choiceId: string): void {
    const next = structuredClone(saveRef.current);
    const history = next.world.eventHistory.find((entry) => entry.id === historyId);
    const packageId = history ? workshopPackageIdFromEvent(next.world.eventDefs[history.eventId]) : undefined;
    if (packageId) {
      const record = enabledWorkshopPackages.find((candidate) => candidate.id === packageId);
      if (!record) { setFeedback({ tone: 'error', text: '对应工坊包未启用，不能执行事件选择。' }); return; }
      const workshopResult = runWorkshopEventChoice(record, historyId, choiceId, next);
      if (!workshopResult.ok) { setFeedback({ tone: 'error', text: workshopResult.message }); return; }
      commitSave(next);
      if (workshopResult.changes.length) promptEvents.emit('onOpsApply', { changes: workshopResult.changes });
      setFeedback({ tone: workshopResult.warnings.length ? 'info' : 'success', text: workshopResult.message });
      return;
    }
  ...
  function moveToNode(nodeId: string): boolean {
    ...
    const participantIds = encounter.triggered ? encounter.candidates.map((candidate) => candidate.id) : [];
    setEncounterParticipantIds(participantIds);
    setEncounterPrimaryId(participantIds.length === 1 ? participantIds[0] : '');
  ...
  async function generateTopicTree(charId: string, nodeId: string, participantIds: string[], encounterEntryId = chatEncounterEntryId): Promise<void> {
    if (!isFormalEncounterParticipant(saveRef.current.world, charId)) {
      setTopicTree(null);
      setTopicMode('choice');
      setFeedback({ tone: 'info', text: '半正式 NPC 暂不开放话题树，可以选择自由聊天。' });
      return;
    }
    ...
    if (existing && isTopicTreeFresh(existing, saveRef.current.world.clock.day)) {
      ...
      setTopicMode('topics');
      setTopicRetryContext(null);
      writeEncounterChatSession({ characterId: charId, participantIds, nodeId, mode: 'topics', ...(encounterEntryId ? { entryId: encounterEntryId } : {}) });
      return;
    }
    const routedProvider = mockFixtureId
      ? createMockProviderConfig(mockFixtureId)
    ...
    if (!routedProvider) {
      setTopicTree(null); setTopicMode('choice'); setTopicRetryContext({ charId, nodeId, participantIds, ...(encounterEntryId ? { entryId: encounterEntryId } : {}) });
      writeEncounterChatSession({ characterId: charId, participantIds, nodeId, mode: 'choice', ...(encounterEntryId ? { entryId: encounterEntryId } : {}) });
      setFeedback({ tone: 'info', text: '未配置话题树 Provider。你仍可自由聊天，或配置后重试。' });
      return;
    }
    let parsed: ProviderConfig;
    try { parsed = ProviderConfigSchema.parse(routedProvider); }
    catch (error) { setTopicTree(null); setTopicMode('choice'); setTopicRetryContext({ charId, nodeId, participantIds, ...(encounterEntryId ? { entryId: encounterEntryId } : {}) }); setFeedback({ tone: 'error', text: errorMessage(error, '话题树 Provider 配置无效。') }); return; }
    setTopicLoading(true); setTopicMode('topics'); setTopicRetryContext(null); setFeedback({ tone: 'info', text: '正在生成本次相遇的话题树…' });
    try {
      const mainCharacter = encounterPromptParticipant(saveRef.current.world, characters, charId);
      const scene = saveRef.current.world.map.nodes[nodeId];
      const participants = encounterPromptParticipants(saveRef.current.world, characters, participantIds);
      const activePresetBundle = presetBundles.find((item) => item.id === selectedPresetBundleId);
      const relationshipState = mainCharacter?.tier === 'formal' ? deriveRelationshipPromptState(saveRef.current.world, mainCharacter.id, saveRef.current.config.stageRules, saveRef.current.config.showNumbers) : undefined;
      const promptFacts = {
        input: '',
      ...
      setTopicTree(null); setTopicMode('choice'); setTopicRetryContext({ charId, nodeId, participantIds, ...(encounterEntryId ? { entryId: encounterEntryId } : {}) }); writeEncounterChatSession({ characterId: charId, participantIds, nodeId, mode: 'choice', ...(encounterEntryId ? { entryId: encounterEntryId } : {}) }); setFeedback({ tone: 'info', text: `话题树生成失败：${errorMessage(error, '生成失败')}。你可以自由聊天或稍后重试。` });
  ...
  function selectTopic(topic: Topic): void {
    ...
    if (!remaining) { setTopicMode('choice'); writeEncounterChatSession({ characterId: selectedCharacterId, participantIds: chatParticipantIds, nodeId: next.world.player.nodeId, mode: 'choice', entryId: chatEncounterEntryId || undefined, lastResponseSource: 'topic' }); setFeedback({ tone: 'info', text: '当前话题已经聊完，可以重新选择接下来的互动。' }); }
  }

  async function generateEncounterOpening(session: EncounterChatSession, retry = false): Promise<void> {
    const requestKey = session.entryId || `${session.nodeId}:${session.characterId}`;
    if (openingRequestRef.current === requestKey || (!retry && readEncounterChatSession()?.mode !== 'opening')) return;
    ...
    const finish = async (content: string, retryAvailable: boolean): Promise<void> => {
      const record = await loadChat(session.characterId);
      const completed = upsertOpening(record?.messages ?? [], content);
      await saveChat({ characterId: session.characterId, messages: completed, updatedAt: now() });
      if (selectedCharacterIdRef.current === session.characterId) setMessages(completed);
      setOpeningRetryAvailable(retryAvailable);
      setTopicMode('choice');
      writeEncounterChatSession({ ...session, mode: 'choice' });
    };
    const routedProvider = mockFixtureId
      ? createMockProviderConfig(mockFixtureId)
      : resolveProviderForCharacter(providers, bindings, characterBindings, current.meta.id, session.characterId, 'narrate_main', defaultProviderId);
    let parsed: ProviderConfig;
    try {
      if (!routedProvider) throw new Error('未配置可用的叙述 Provider。');
      parsed = ProviderConfigSchema.parse(routedProvider);
    } catch (error) {
      await finish(fallback, true);
      setFeedback({ tone: 'info', text: `${errorMessage(error, 'Provider 配置不可用')} 已使用本地到场描述；可以继续互动或稍后重试开场。` });
      openingRequestRef.current = '';
      return;
    }
    setBusy(true); setReplyInProgress(true); setRequestStatus('requesting'); setOpeningRetryAvailable(false);
    const participants = encounterPromptParticipants(current.world, characters, session.participantIds);
    const mainCharacter = encounterPromptParticipant(current.world, characters, session.characterId);
    const firstEncounter = current.world.encounterLog.filter((entry) => entry.characterIds.includes(session.characterId)).length <= 1;
    const activePresetBundle = presetBundles.find((item) => item.id === selectedPresetBundleId);
    const relationshipState = mainCharacter?.tier === 'formal' ? deriveRelationshipPromptState(current.world, mainCharacter.id, current.config.stageRules, current.config.showNumbers) : undefined;
    ...
    try {
      ...
      if (!narrative.trim()) throw new Error('Provider 未返回可读的开场正文。');
      await finish(narrative, false);
      setDebug((debugState) => ({ ...debugState, raw: finished.raw, ops: JSON.stringify({ discarded: finished.foundOps, message: '开场只保留叙述文字，不解析或应用状态操作。' }, null, 2) }));
      setFeedback({ tone: 'success', text: '开场已生成。接下来可以自由选择互动方式。' });
      void showGenerationCompleteNotification();
    } catch (error) {
      splitter.finish();
      await finish(fallback, true);
      setRequestStatus('error');
      setFeedback({ tone: 'info', text: `开场生成失败，已使用本地到场描述：${errorMessage(error, '请求失败')}。可以继续互动或显式重试。` });
    } finally {
      openingRequestRef.current = '';
      setBusy(false); setReplyInProgress(false);
    }
  }

  function chooseSceneMode(mode: 'choice' | 'topics' | 'manual'): void {
    if (!selectedCharacterId || busy) return;
    const session = readEncounterChatSession();
    if (!session) return;
    if (mode === 'topics') {
      void generateTopicTree(selectedCharacterId, session.nodeId, session.participantIds, session.entryId);
      return;
    }
    setTopicMode(mode);
  ...
  function retryEncounterOpening(): void {
    ...
    setTopicMode('opening');
    writeEncounterChatSession({ ...session, mode: 'opening' });
    void generateEncounterOpening({ ...session, mode: 'opening' }, true);
  }
  ...
  function continueEncounter(): void {
    ...
    if (!result.ok) { setFeedback({ tone: 'error', text: result.warning ?? '无法记录相遇结果。' }); return; }
    const availableIds = new Set(activeEncounter.candidates.map((candidate) => candidate.id).filter((id) => Boolean(saveRef.current.world.characters[id] || saveRef.current.world.npcs[id])));
    if (!availableIds.size) {
      ...
      setEncounterPrimaryId('');
      writeEncounterChatSession(null);
      setFeedback({ tone: 'info', text: '你决定留下继续，但当前没有可互动的人物资料。' });
      return;
    }
    const participantIds = encounterParticipantIds.filter((id) => availableIds.has(id));
    if (!participantIds.length) { setFeedback({ tone: 'error', text: '请至少选择一位人物进入场景。' }); return; }
    if (!encounterPrimaryId || !participantIds.includes(encounterPrimaryId)) { setFeedback({ tone: 'error', text: '多人场景需要明确选择一位主要互动对象。' }); return; }
    const primary = encounterPromptParticipant(next.world, characters, encounterPrimaryId);
    commitSave(next);
    setActiveEncounter(null);
    setEncounterParticipantIds([]);
    setEncounterPrimaryId('');
    if (!primary) { setFeedback({ tone: 'info', text: '你决定留下继续，但主要互动对象的资料已不可用。' }); return; }
    setChatParticipantIds(participantIds);
    setChatEncounterEntryId(activeEncounter.entryId);
    setChatParticipantsLocked(true);
    markResponseSource(null);
    selectedCharacterIdRef.current = primary.id;
    setSelectedCharacterId(primary.id);
    setTopicTree(null);
    setTopicRetryContext(null);
    setTopicMode('opening');
    setOpeningRetryAvailable(false);
    const session: EncounterChatSession = { characterId: primary.id, participantIds, nodeId: next.world.player.nodeId, mode: 'opening', entryId: activeEncounter.entryId };
    writeEncounterChatSession(session);
    setTab('chat');
    void generateEncounterOpening(session);
    setFeedback({ tone: 'info', text: `你留下来和${primary.name}继续这次相遇，正在生成开场。` });
  }
  ...
  async function sayGoodbye(): Promise<void> {
    ...
    writeEncounterChatSession({ characterId: selectedCharacterId, participantIds: chatParticipantIds, nodeId: next.world.player.nodeId, mode: 'ended', entryId: chatEncounterEntryId });
    setBusy(true);
    const formal = Boolean(next.world.characters[selectedCharacterId]);
    setFeedback({ tone: 'info', text: formal ? '你主动结束了这次相遇，正在整理本次对话…' : '你主动结束了这次相遇。NPC 聊天文字已保留，但不会自动写入关系或轻记忆。' });
    try { if (formal) await consolidateEncounterMemories(next, selectedCharacterId, messages); }
  ...
  async function generateReply(giftContext?: GiftGenerationContext, providedMessages?: ChatMessage[], suppressItemGains = false, collectionContext?: CollectionGenerationContext) {
    ...
    const npcLight = Boolean(saveRef.current.world.npcs[generationCharacterId] && !saveRef.current.world.characters[generationCharacterId]);
    const routedProvider = mockFixtureId
      ? createMockProviderConfig(mockFixtureId)
    ...
    const participants = encounterPromptParticipants(saveRef.current.world, characters, participantIds);
    const generationCharacter = encounterPromptParticipant(saveRef.current.world, characters, generationCharacterId);
    const relationshipState = generationCharacter?.tier === 'formal' ? deriveRelationshipPromptState(saveRef.current.world, generationCharacter.id, saveRef.current.config.stageRules, saveRef.current.config.showNumbers) : undefined;
    const relationshipMemories = npcLight ? undefined : await relationshipMemoryOverride(latestInput, generationCharacterId);
    const promptFacts = { input: latestInput, character: generationCharacter, participants, presetBundle: activePresetBundle, playerPersona: activePersona, relationshipState, relationshipMemories, interactionPolicy: npcLight ? 'npc-light' as const : undefined, giftContext: giftContext ? { ...giftContext, description: saveRef.current.world.items[giftContext.itemId]?.description, tags: saveRef.current.world.items[giftContext.itemId]?.tags ?? [] } : undefined, collectionContext, worldbooks, history: next, world: saveRef.current.world };
    promptEvents.emit('beforePromptAssemble', { facts: promptFacts, task: 'narrate_main' });
    const assembled = assembler.assemble(promptFacts, { budget: Math.max(1, parsed.contextWindow - parsed.maxOutputTokens), task: 'narrate_main' });
    setDebug((current) => ({ ...current, prompt: assembled }));
    try {
      ...
      if (npcLight) {
        const reply = await parseReply(finished.raw);
        updateChatRecovery(null);
        setPendingOps(null);
        setDebug((current) => ({ ...current, raw: finished.raw, ops: JSON.stringify({ stage: reply.stage, discardedOps: reply.ops, warnings: [...reply.warnings, ...(finished.foundOps ? ['半正式 NPC 轻互动中的所有 ops 均已拒绝。'] : [])], message: 'NPC 轻互动只保存聊天文字，不写入任何世界事实。' }, null, 2) }));
        if (finished.foundOps) setFeedback({ tone: 'info', text: 'NPC 回复已保存；附带的状态操作已全部拒绝。' });
      } else {
        const parsedReply = await parseReply(finished.raw, extractOps);
        const npcBoundary = rejectNpcTargetedOps(parsedReply.ops, participantIds.filter((id) => Boolean(saveRef.current.world.npcs[id])));
        const reply = { ...parsedReply, ops: npcBoundary.ops, warnings: [...parsedReply.warnings, ...(npcBoundary.rejected ? [`混合场景中 ${npcBoundary.rejected} 个面向半正式 NPC 的状态操作已拒绝。`] : [])] };
  ...
  async function regenerateReply(): Promise<void> {
    ...
    const participantIds = chatParticipantIds.length ? chatParticipantIds : [selectedCharacterId];
    const participants = encounterPromptParticipants(saveRef.current.world, characters, participantIds);
    const promptCharacter = encounterPromptParticipant(saveRef.current.world, characters, selectedCharacterId);
    const npcLight = promptCharacter?.tier === 'semi';
    const latestInput = [...baseMessages].reverse().find((message) => message.role === 'user')?.content ?? '';
    const relationshipState = promptCharacter?.tier === 'formal' ? deriveRelationshipPromptState(saveRef.current.world, promptCharacter.id, saveRef.current.config.stageRules, saveRef.current.config.showNumbers) : undefined;
    const relationshipMemories = npcLight ? undefined : await relationshipMemoryOverride(latestInput, selectedCharacterId);
    const promptFacts = { input: latestInput, regenerationRequest: requirement, character: promptCharacter, participants, presetBundle: activePresetBundle, playerPersona: activePersona, relationshipState, relationshipMemories, interactionPolicy: npcLight ? 'npc-light' as const : undefined, worldbooks, history: originalMessages, world: saveRef.current.world };
    promptEvents.emit('beforePromptAssemble', { facts: promptFacts, task: 'narrate_main' });
    const assembled = assembler.assemble(promptFacts, { budget: Math.max(1, parsed.contextWindow - parsed.maxOutputTokens), task: 'narrate_main' });
    setDebug((current) => ({ ...current, prompt: assembled }));
  ...
  async function generateWorkshopPackageDraft(requirement: string, signal?: AbortSignal) {
    const routedProvider = mockFixtureId
      ? createMockProviderConfig(mockFixtureId)
      : resolveProviderForTask(providers, bindings, 'workshop_draft', defaultProviderId);
    if (!routedProvider) throw new Error('请先保存并设置默认 Provider，或为“工坊 Agent”配置独立路由。');
    const parsed = ProviderConfigSchema.parse(routedProvider);
    return requestWorkshopDraft(parsed, requirement, { signal });
  }

  async function runWorkshopPackageAgentTurn(input: WorkshopAgentTurnInput, signal?: AbortSignal) {
    const routedProvider = mockFixtureId
      ? createMockProviderConfig(mockFixtureId)
      : resolveProviderForTask(providers, bindings, 'workshop_draft', defaultProviderId);
    if (!routedProvider) throw new Error('请先保存并设置默认 Provider，或为“工坊 Agent”配置独立路由。');
    return requestWorkshopAgentTurn(ProviderConfigSchema.parse(routedProvider), input, { signal });
  }

  async function runWorkshopPackageProviderText(packageId: string, action: WorkshopProviderTextAction, values: Readonly<Record<string, WorkshopLocalValue>>, signal?: AbortSignal): Promise<{ ok: boolean; message: string; text?: string; resultKey?: string }> {
    try {
      const record = enabledWorkshopPackages.find((candidate) => candidate.id === packageId);
      if (!record) throw new Error('工坊包未安装或未为当前世界启用。');
      const prepared = prepareWorkshopProviderText(record, action, { world: saveRef.current.world }, values);
      const routedProvider = mockFixtureId
        ? createMockProviderConfig(mockFixtureId)
        : resolveProviderForTask(providers, bindings, prepared.taskId, defaultProviderId);
      if (!routedProvider) throw new Error(`请先设置默认 Provider，或为任务 ${prepared.taskId} 配置独立路由。`);
      const text = await requestWorkshopProviderText(ProviderConfigSchema.parse(routedProvider), prepared.messages, prepared.taskId, {
        signal,
        onStatus: (status) => setRequestStatus(status),
      });
      const stored = prepared.resultKey ? text.slice(0, 2000) : undefined;
      const truncated = stored !== undefined && stored.length < text.length;
      return {
        ok: true,
        message: prepared.resultKey ? `Provider 已返回，结果已写入本地字段 ${prepared.resultKey}${truncated ? '（按本地状态上限保留前 2000 字符）' : ''}。` : 'Provider 已返回文本；未写入世界状态。',
        text,
        ...(prepared.resultKey ? { resultKey: prepared.resultKey } : {}),
      };
    } catch (error) {
      setRequestStatus('error');
      return { ok: false, message: errorMessage(error, '工坊 Provider 请求失败；未写入任何世界状态。') };
    }
  }

  async function runWorkshopPackageActivity(packageId: string, ruleId: string): Promise<{ ok: boolean; message: string }> {
    if (ruleId.startsWith('@event:')) return runWorkshopPackageEvent(packageId, ruleId.slice('@event:'.length));
    const record = enabledWorkshopPackages.find((candidate) => candidate.id === packageId);
    if (!record) return { ok: false, message: '活动包未安装或未为当前世界启用。' };
    const next = structuredClone(saveRef.current);
    const applied = runWorkshopActivity(record, ruleId, 'manual', {
      world: next.world,
      day: next.world.clock.day,
      slotId: next.world.clock.slotId,
      nodeId: next.world.player.nodeId,
  ...
  async function runWorkshopPackageEvent(packageId: string, eventId: string): Promise<{ ok: boolean; message: string }> {
    const record = enabledWorkshopPackages.find((candidate) => candidate.id === packageId);
    if (!record) return { ok: false, message: '事件包未安装或未为当前世界启用。' };
    const next = structuredClone(saveRef.current);
    const result = runWorkshopEvent(record, eventId, next);
    if (!result.ok) return { ok: false, message: result.message };
    commitSave(next);
    if (result.changes.length) promptEvents.emit('onOpsApply', { changes: result.changes });
    setFeedback({ tone: result.warnings.length ? 'info' : 'success', text: result.message });
  ...
  async function addContent(kind: ContentKind) {
    ...
    if (kind === 'worldbook') { const existing = worldbooks.find((item) => item.id === id); const item = await saveWorldbook({ id, name, content: draftText, keys: parseWorldbookKeys(worldbookKeys), enabled: existing?.enabled ?? true, priority: existing?.priority ?? 50 }); setWorldbooks((items) => [...items.filter((old) => old.id !== id), item]); }
    if (kind === 'preset') {
    ...
    setName(''); setDraftText(''); setWorldbookKeys(''); setEditing(null); setFeedback({ tone: 'success', text: '内容已保存。' });
  }
  ...
  async function downloadGlobalBackup(includeSecrets: boolean): Promise<void> {
    try {
      ...
      const [currentSave, snapshots, characters, personas, worldbooks, presets, presetBundles, storyScenePresets, chats, chatRecovery, memoryVectors, musicStates, terminalStickers, workshopPackages, workshopBindings, workshopStates, ps, tts, bs, cbs, imageConfigs, imageVisuals, imageUserVisuals, settings] = await Promise.all([loadCurrentSave(), listSnapshots(), contentDb.characters.toArray(), contentDb.personas.toArray(), contentDb.worldbooks.toArray(), contentDb.presets.toArray(), contentDb.presetBundles.toArray(), contentDb.storyScenePresets.toArray(), contentDb.chats.toArray(), contentDb.chatRecovery.toArray(), contentDb.memoryVectors.toArray(), contentDb.musicStates.toArray(), contentDb.terminalStickers.toArray(), contentDb.workshopPackages.toArray(), contentDb.workshopBindings.toArray(), contentDb.workshopStates.toArray(), providerDb.providers.toArray(), providerDb.ttsConfigs.toArray(), providerDb.bindings.toArray(), providerDb.characterBindings.toArray(), providerDb.imageConfigs.toArray(), providerDb.imageVisualConfigs.toArray(), providerDb.imageUserVisualConfigs.toArray(), providerDb.settings.toArray()]);
      const localStorage = collectTokimekiPreferences(window.localStorage);
      const blob = await exportGlobalBackup({ currentSave, snapshots, content: { characters, personas, worldbooks, presets, presetBundles, storyScenePresets, chats, chatRecovery, memoryVectors, musicStates, terminalStickers, workshopPackages, workshopBindings, workshopStates }, providers: ps, ttsConfigs: tts, bindings: bs, characterBindings: cbs, imageConfigs, imageVisualConfigs: imageVisuals, imageUserVisualConfigs: imageUserVisuals, settings, localStorage, theme: collectThemeBackup(window.localStorage) }, assets, includeSecrets);
      ...
      setFeedback({ tone: 'success', text: `全局备份已导出，包含 ${workshopPackages.length} 个工坊包、${workshopStates.length} 份本地 App 状态和 ${assets.length} 个资产${includeSecrets ? '（含 API key，请勿分享）' : '（不含 API key）'}。` });
  ...
  async function previewGlobalBackup(file?: File): Promise<void> {
    ...
    try {
      ...
      setFeedback({ tone: backup.workshopIntegrity.issues.some((issue) => issue.severity === 'error') ? 'info' : 'success', text: `已读取全局备份：${backup.data.content.workshopPackages.length} 个工坊包、${backup.data.content.workshopStates.length} 份本地 App 状态、${backup.data.providers.length} 个普通 Provider、${backup.data.ttsConfigs.length} 个语音 Provider、${backup.assets.size} 个资产${backup.workshopIntegrity.issues.length ? `；工坊完整性有 ${backup.workshopIntegrity.issues.length} 项提示` : ''}${backup.hasSecrets ? '，包含 API key' : ''}。尚未写入本地数据。` });
  ...
  async function restoreGlobalBackup(backup: ImportedGlobalBackup, selection: GlobalBackupRestoreSelection = { world: true, content: true, providers: true, assets: true, preferences: true }): Promise<void> {
    const workshopErrorCount = backup.workshopIntegrity.issues.filter((issue) => issue.severity === 'error').length;
    const workshopWarning = selection.content && workshopErrorCount
      ? `\n\n备份中的工坊数据有 ${workshopErrorCount} 项错误。原记录会保留，但依赖或资源不完整的包将由运行时失败关闭；不会自动修复或删除用户数据。`
      : selection.content && !selection.assets && backup.workshopIntegrity.referencedAssetCount
        ? `\n\n当前未选择恢复资产，备份中的 ${backup.workshopIntegrity.referencedAssetCount} 项工坊资产映射可能无法使用。不会自动删除包或本地 App 状态。`
        : '';
    if (!window.confirm(`恢复所选类别会覆盖当前浏览器中的对应本地数据。${workshopWarning}\n\n确定继续吗？`)) return;
    const previous = {
      current: selection.world ? await loadCurrentSave() : undefined,
      snapshots: selection.world ? await listSnapshots() : [],
      content: selection.content ? { characters: await contentDb.characters.toArray(), personas: await contentDb.personas.toArray(), worldbooks: await contentDb.worldbooks.toArray(), presets: await contentDb.presets.toArray(), presetBundles: await contentDb.presetBundles.toArray(), storyScenePresets: await contentDb.storyScenePresets.toArray(), chats: await contentDb.chats.toArray(), chatRecovery: await contentDb.chatRecovery.toArray(), memoryVectors: await contentDb.memoryVectors.toArray(), musicStates: await contentDb.musicStates.toArray(), terminalStickers: await contentDb.terminalStickers.toArray(), workshopPackages: await contentDb.workshopPackages.toArray(), workshopBindings: await contentDb.workshopBindings.toArray(), workshopStates: await contentDb.workshopStates.toArray() } : undefined,
    ...
    try {
      ...
      if (selection.content) { await Promise.all([contentDb.characters.clear(), contentDb.personas.clear(), contentDb.worldbooks.clear(), contentDb.presets.clear(), contentDb.presetBundles.clear(), contentDb.storyScenePresets.clear(), contentDb.chats.clear(), contentDb.chatRecovery.clear(), contentDb.memoryVectors.clear(), contentDb.musicStates.clear(), contentDb.terminalStickers.clear(), contentDb.workshopPackages.clear(), contentDb.workshopBindings.clear(), contentDb.workshopStates.clear()]); await Promise.all([contentDb.characters.bulkPut(backup.data.content.characters), contentDb.personas.bulkPut(backup.data.content.personas), contentDb.worldbooks.bulkPut(backup.data.content.worldbooks), contentDb.presets.bulkPut(backup.data.content.presets), contentDb.presetBundles.bulkPut(backup.data.content.presetBundles), contentDb.storyScenePresets.bulkPut(backup.data.content.storyScenePresets), contentDb.chats.bulkPut(backup.data.content.chats), contentDb.chatRecovery.bulkPut(backup.data.content.chatRecovery), contentDb.memoryVectors.bulkPut(backup.data.content.memoryVectors), contentDb.musicStates.bulkPut(backup.data.content.musicStates), contentDb.terminalStickers.bulkPut(backup.data.content.terminalStickers), contentDb.workshopPackages.bulkPut(backup.data.content.workshopPackages), contentDb.workshopBindings.bulkPut(backup.data.content.workshopBindings), contentDb.workshopStates.bulkPut(backup.data.content.workshopStates)]); }
  ...
  async function checkAssetIntegrity(): Promise<void> {
    ...
    try {
      const [assets, roots, workshopPackages, workshopBindings, workshopStates] = await Promise.all([listAssets(), loadImageAssetReferenceRoots(), contentDb.workshopPackages.toArray(), contentDb.workshopBindings.toArray(), contentDb.workshopStates.toArray()]);
      ...
      const workshopReport = auditWorkshopIntegrity(workshopPackages, workshopBindings, workshopStates, new Set(assets.filter((asset) => asset.blob.size > 0).map((asset) => asset.id)));
      setAssetIntegrityReport(report);
      setWorkshopIntegrityReport(workshopReport);
      const workshopErrorCount = workshopReport.issues.filter((issue) => issue.severity === 'error').length;
      const problemCount = report.missing.length + workshopErrorCount;
      setFeedback({ tone: problemCount ? 'info' : 'success', text: problemCount ? `检查完成：发现 ${report.missing.length} 个缺失或空资产引用、${workshopErrorCount} 项工坊数据错误。未自动修改任何数据。` : '本地数据完整性检查完成，未发现失效引用或工坊数据错误。' });
    } catch (error) {
  ...
  const libraryDayPage = libraryPage && !workshopPackageIdFromRoute(libraryPage) ? LIBRARY_DAY_PAGE_MAP[libraryPage as LibraryPage] : undefined;
...
function MapView({ save, worldbooks, activeEncounter, encounterParticipantIds, encounterPrimaryId, onEncounterParticipantIdsChange, onEncounterPrimaryIdChange, onEncounterOutcome, onContinueEncounter, onMove, onImportBackground, onSetBackgroundUrl, onImportSceneBackground, onSetSceneBackgroundUrl, onRemoveSceneBackground, onToggleMode, onCreateNode, onEditNode, onDeleteNode, onSuggestNode, onGenerateMap, onExpandMap, mapGenerating, onCreatePlaceHighlight, onUpdatePlaceHighlight, onDeletePlaceHighlight, onGeneratePlaceHighlights, onConfirmPlaceHighlights, placeHighlightGenerating, onStartActivity }: MapViewProps) {
  ...
  return <section className="map-screen">
    ...
    {activeEncounter && <EncounterDialog encounter={activeEncounter} selectedParticipantIds={encounterParticipantIds} primaryId={encounterPrimaryId} onSelectionChange={onEncounterParticipantIdsChange} onPrimaryChange={onEncounterPrimaryIdChange} onOutcome={onEncounterOutcome} onContinue={onContinueEncounter} />}
...
function EncounterDialog({ encounter, selectedParticipantIds, primaryId, onSelectionChange, onPrimaryChange, onOutcome, onContinue }: { encounter: ActiveEncounter; selectedParticipantIds: string[]; primaryId: string; onSelectionChange: (ids: string[]) => void; onPrimaryChange: (id: string) => void; onOutcome: (outcome: 'continued' | 'urgent_leave') => void; onContinue: () => void }) {
  const names = encounter.candidates.map((candidate) => candidate.name).join('、');
  ...
  const selected = encounter.candidates.filter((candidate) => selectedParticipantIds.includes(candidate.id));
  return <div className="encounter-dialog" role="dialog" aria-label="相遇事件"><div className="encounter-dialog-copy"><span className="eyebrow">有人可遇</span><strong>你在这里遇见了{names}</strong><p>{encounter.scope === 'formal' ? '地点正在开放，可以正式进入范围。' : '地点尚未开放，你们只能在附近外围短暂相遇。'}</p></div><div className="encounter-participant-picker"><strong>选择在场人物</strong><span className="io-scope">正式角色与半正式 NPC 都可参与；确认后固定到场景结束</span><div>{encounter.candidates.map((candidate) => <label key={candidate.id}><input type="checkbox" checked={selectedParticipantIds.includes(candidate.id)} onChange={() => toggle(candidate.id)} /><span>{candidate.name}<small>{candidate.tier === 'formal' ? '正式角色' : '半正式 NPC'}</small></span></label>)}</div></div>{selected.length > 1 && <div className="encounter-primary-picker"><strong>主要互动对象</strong><span className="io-scope">开场和回复以此人为主</span>{selected.map((candidate) => <label key={candidate.id}><input type="radio" name="encounter-primary" checked={primaryId === candidate.id} onChange={() => onPrimaryChange(candidate.id)} />{candidate.name}</label>)}</div>}<div className="encounter-options"><button onClick={onContinue} disabled={!selected.length || !primaryId}>留下并对话</button><button className="secondary" onClick={() => onOutcome('urgent_leave')}>离开</button></div></div>;
}
...
function ChatView(props: {
  characters: EncounterPromptParticipant[];
  ...
  topicMode: EncounterSceneMode;
  topicLoading: boolean;
  topicRetryAvailable: boolean;
  onRetryTopicTree: () => void;
  openingRetryAvailable: boolean;
  onRetryOpening: () => void;
  onChooseSceneMode: (mode: 'choice' | 'topics' | 'manual') => void;
  hasFormalPrimary: boolean;
  ...
  const speakerIdsByName = { [props.playerLabel]: 'player', ...Object.fromEntries(props.characters.map((character) => [character.name, character.id])) };
  const speakerLabelsById = { player: props.playerLabel, ...Object.fromEntries(props.characters.map((character) => [character.id, character.name])) };
  ...
  const canRevealNextLine = !props.busy && latestRole === 'assistant' && latestAssistantLines.length > effectiveRevealedLineCount;
  ...
  const revealNextLine = () => {
    ...
    setRevealedLineCount(Math.min(latestAssistantLines.length, effectiveRevealedLineCount + 1));
  };
  const nextLineAction = canRevealNextLine ? <button type="button" className="vn-next-line" aria-label={`下一段 ${effectiveRevealedLineCount}/${latestAssistantLines.length}`} title="下一段" onClick={revealNextLine}><ChevronDown aria-hidden="true" /><span>下一段 · {effectiveRevealedLineCount}/{latestAssistantLines.length}</span></button> : null;
  ...
  return <section className="chat-screen vn-chat-screen">
    ...
    {props.topicMode === 'choice' && <div className="scene-choice-panel" aria-label="场景选择"><div className="list-heading"><strong>接下来做什么？</strong>{props.openingRetryAvailable && <button className="secondary" onClick={props.onRetryOpening} disabled={props.busy}>重试开场</button>}</div><div className="scene-choice-actions">{nextLineAction}{props.hasFormalPrimary && <button onClick={() => props.onChooseSceneMode('topics')} disabled={props.busy}>看看话题</button>}<button onClick={() => props.onChooseSceneMode('manual')} disabled={props.busy}>自由聊</button>{props.canFarewell && <button className="secondary" onClick={props.onPlayerFarewell} disabled={props.busy}>离开</button>}</div>{!props.hasFormalPrimary && <span className="io-scope">半正式 NPC 提供轻量自由聊天；完整关系能力需先在联系人页确认转正。</span>}{props.topicRetryAvailable && <button className="secondary" onClick={props.onRetryTopicTree} disabled={props.busy || props.topicLoading}>重试生成话题树</button>}</div>}
    {props.topicMode === 'topics' && <div className="topic-tree-panel" aria-label="话题树">
      <div className="list-heading"><strong>可以聊聊</strong><div className="button-row">{nextLineAction}<button className="secondary" onClick={() => props.onChooseSceneMode('choice')} disabled={props.busy}>返回选择</button><button className="secondary" onClick={() => props.onChooseSceneMode('manual')} disabled={props.busy}>自由聊天</button>{props.topicLoading && <span className="request-status requesting">正在生成话题树…</span>}</div></div>
    ...
    {props.topicMode === 'manual' && <div className="scene-mode-bar"><button className="secondary" onClick={() => props.onChooseSceneMode('choice')} disabled={props.busy}>场景选择</button>{props.hasFormalPrimary && <button className="secondary" onClick={() => props.onChooseSceneMode('topics')} disabled={props.busy || props.topicLoading}>看看话题</button>}</div>}
    ...
    {props.topicMode === 'manual' && <div className="interaction-tools" aria-label="聊天操作工具">
      {props.canFarewell && props.formalToolsAvailable && <button type="button" className="interaction-icon chat-icon-button" aria-label="打开送礼" aria-pressed={activeChatPanel === 'gift'} title="送礼" onClick={() => toggleChatPanel('gift')}><Gift aria-hidden="true" /></button>}
      {props.canFarewell && props.hasFormalPrimary && <button type="button" className="interaction-icon chat-icon-button" aria-label="打开收藏" aria-pressed={activeChatPanel === 'collection'} title="出示收藏" onClick={() => toggleChatPanel('collection')}><BookOpen aria-hidden="true" /></button>}
...
function ProviderSettingsMigrationView(props: {
  ...
  onRestoreGlobalBackup: (backup: ImportedGlobalBackup, selection?: GlobalBackupRestoreSelection) => Promise<void>;
}) {
  ...
  const workshopErrors = props.globalBackupPreview?.workshopIntegrity.issues.filter((issue) => issue.severity === 'error') ?? [];
  return <div className="library-subpage-content">
    <section className="library-legacy-content">
      <div className="section-heading"><div><span className="eyebrow">仅本地</span><h2>Provider 设置迁移</h2></div></div>
      <div className="provider-card">
        <h3>全局备份</h3>
        <p className="io-scope">一键备份当前世界、快照、聊天、资料、工坊包、世界绑定、本地 App 状态、Provider 配置、界面偏好和本地资产。默认不包含 API key。</p>
        <div className="button-row">
          <button onClick={() => void props.onExportGlobalBackup(false)}>导出全局备份</button>
          <button className="secondary" onClick={() => { if (window.confirm('完整备份会把 API key 写入文件，请勿分享。确定继续吗？')) void props.onExportGlobalBackup(true); }}>导出完整备份（含 API key）</button>
          <label className="file-button">导入全局备份<input type="file" accept=".zip" onChange={(event) => void props.onImportGlobalBackup(event.target.files?.[0])} /></label>
        </div>
        {props.globalBackupPreview && <div className="fold-body">
          <p>备份预览：{props.globalBackupPreview.data.providers.length} 个普通 Provider、{props.globalBackupPreview.data.ttsConfigs.length} 个语音 Provider、{props.globalBackupPreview.data.imageConfigs.length} 份图像设置、{props.globalBackupPreview.data.imageVisualConfigs.length + props.globalBackupPreview.data.imageUserVisualConfigs.length} 份视觉配置、{props.globalBackupPreview.data.content.workshopPackages.length} 个工坊包、{props.globalBackupPreview.data.content.workshopBindings.length} 条世界绑定、{props.globalBackupPreview.data.content.workshopStates.length} 份本地 App 状态、{props.globalBackupPreview.assets.size} 个资产{props.globalBackupPreview.hasSecrets ? '，包含 API key' : ''}。</p>
          <div className="stat-list">
            <span>{props.globalBackupPreview.workshopIntegrity.issues.length} 项工坊完整性提示</span>
            <span>{workshopErrors.length} 项错误</span>
            <span>{props.globalBackupPreview.workshopIntegrity.retainedLocalStateCount} 份卸载后保留状态</span>
          </div>
          {workshopErrors.length > 0 && <p className="io-scope" role="alert">恢复会保留原始记录，不自动修复或删除用户数据；依赖或资源不完整的包会由运行时失败关闭。</p>}
          {props.globalBackupPreview.workshopIntegrity.issues.length > 0 && <details><summary>查看工坊完整性提示</summary><div className="integrity-issue-list">{props.globalBackupPreview.workshopIntegrity.issues.map((issue, index) => <div key={`${issue.code}:${issue.packageId ?? issue.recordId ?? index}`}><strong>{issue.severity === 'error' ? '错误' : '警告'} · {issue.code}</strong><small>{issue.message}</small></div>)}</div></details>}
          {([['world', '世界存档与快照'], ['content', '资料、聊天、工坊包与本地 App 状态'], ['providers', 'Provider 与绑定'], ['assets', '图片、音频及工坊二进制资产'], ['preferences', '界面偏好']] as const).map(([key, label]) => <label className="checkbox-line" key={key}><input type="checkbox" checked={restoreSelection[key]} onChange={(event) => setRestoreSelection((current) => ({ ...current, [key]: event.target.checked }))} />{label}</label>)}
          <button onClick={() => void props.onRestoreGlobalBackup(props.globalBackupPreview!, restoreSelection)}>确认恢复所选内容</button>
        </div>}
      </div>
      <div className="provider-card">
        <p className="io-scope">迁移非敏感配置、默认项、路由绑定和图像生成选项。默认不导出 API key 或鉴权 headers；导入同 ID 配置时保留本机已有密钥。</p>
        <h3>普通 Provider</h3>
        {availableProviders.length ? availableProviders.map((item) => <label className="checkbox-line" key={item.id}><input type="checkbox" checked={providerIds.includes(item.id)} onChange={(event) => toggle(providerIds, item.id, event.target.checked, setProviderIds)} />{item.name} · {item.model}</label>) : <p className="empty">没有普通 Provider 配置。</p>}
        <h3>语音 Provider</h3>
        {availableTtsConfigs.length ? availableTtsConfigs.map((item) => <label className="checkbox-line" key={item.id}><input type="checkbox" checked={ttsIds.includes(item.id)} onChange={(event) => toggle(ttsIds, item.id, event.target.checked, setTtsIds)} />{item.name} · {item.model || '未设置模型'}</label>) : <p className="empty">没有语音 Provider 配置。</p>}
        <label className="checkbox-line"><input type="checkbox" checked={includeBindings} onChange={(event) => setIncludeBindings(event.target.checked)} />包含任务路由和当前世界角色绑定</label>
        <div className="button-row">
          <button onClick={() => { if (includeSecrets && !window.confirm('导出文件将包含 API key 和鉴权 headers。请确认文件只保存在你自己的设备上。')) return; void props.onExport(selection); }} disabled={!providerIds.length && !ttsIds.length && !props.imageConfig}>导出所选设置</button>
          <label className="file-button">选择迁移包<input type="file" accept=".json,application/json" onChange={async (event) => { const file = event.target.files?.[0]; if (!file) return; try { const pack = await importProviderSettingsPackage(file); setPreview(pack); setImportError(''); setProviderIds(pack.providers.map((item) => item.id)); setTtsIds(pack.ttsConfigs.map((item) => item.id)); setIncludeBindings(true); setIncludeSecrets(Boolean(pack.providers.some((item) => item.apiKey) || pack.ttsConfigs.some((item) => item.apiKey))); } catch (error) { setPreview(null); setImportError(errorMessage(error, '迁移包无法读取。')); } }} /></label>
          <label className="checkbox-line"><input type="checkbox" checked={includeSecrets} onChange={(event) => setIncludeSecrets(event.target.checked)} />包含 API key 和鉴权 headers（仅用于个人迁移）</label>
          {includeSecrets && <p className="io-scope" role="alert">API key 将写入导出的 JSON 文件。请勿分享此文件；导出前会再次确认。</p>}
        </div>
        {importError && <p className="io-scope" role="alert">{importError}</p>}
      </div>
      {preview && <div className="provider-card">
        <div className="list-heading"><div><h3>导入预览</h3><p className="io-scope">导出时间：{new Date(preview.exportedAt).toLocaleString()}</p></div></div>
        <p>普通 Provider：{preview.providers.length} 个 · 语音 Provider：{preview.ttsConfigs.length} 个 · 任务路由：{preview.bindings.length} 条 · 角色绑定：{preview.characterBindings.length} 条 · 图像选项：{preview.imageConfig ? '包含' : '无'}</p>
        <div className="button-row"><button onClick={() => void props.onImport(preview, selection)} disabled={!providerIds.length && !ttsIds.length && !preview.imageConfig}>导入所选设置</button><button className="secondary" onClick={() => setPreview(null)}>取消预览</button></div>
      </div>}
    </section>
  </div>;
}
...
function SettingsView(props: {
  ...
  workshopIntegrityReport: WorkshopIntegrityReport | null;
  ...
  return <SubpageShell title={pageTitle} pageId={props.activePage} onBack={props.onBack}>
    ...
    {props.activePage === 'player' && <PersonaAvatarSettings saveId={props.save.meta.id} playerName={props.save.world.player.name} personas={props.personas} activePersonaId={props.personaId} onSave={props.onSavePersona} />}
    ...
    <details className="fold-card settings-privacy-page" open><summary>数据与隐私</summary><div className="fold-body"><div className="provider-card">
      ...
      <h3>本地数据完整性</h3>
      <p className="io-scope">检查当前世界、角色库、快照、面对面聊天、贴图库和锁脸配置中的资产引用，并核对工坊包、世界绑定、本地 App 状态、依赖和二进制资产。检查只读取 IndexedDB，不修改、修复或删除任何数据。</p>
      ...
      {props.assetIntegrityReport && <div className="asset-integrity-report">
        <h4>通用资产引用</h4>
        <div className="stat-list"><span>{props.assetIntegrityReport.referenceCount} 处引用</span><span>{props.assetIntegrityReport.referencedAssetCount} 个被引用资产</span><span>{props.assetIntegrityReport.storedAssetCount} 个本地资产</span><span>{props.assetIntegrityReport.missing.length} 个失效引用</span><span>{props.assetIntegrityReport.orphaned.length} 个孤立资产</span></div>
        {props.assetIntegrityReport.missing.length === 0 && props.assetIntegrityReport.orphaned.length === 0 ? <p className="io-scope">引用与本地二进制一致。</p> : <>
          {props.assetIntegrityReport.missing.length > 0 && <details><summary>缺失或空资产</summary><div className="integrity-issue-list">{props.assetIntegrityReport.missing.map((issue) => <div key={issue.assetId}><strong>{issue.assetId}</strong><small>{issue.empty ? '本地记录为零字节' : 'Assets IndexedDB 中不存在'} · {issue.sources.join('；')}</small></div>)}</div></details>}
          {props.assetIntegrityReport.orphaned.length > 0 && <details><summary>无引用资产</summary><div className="integrity-issue-list">{props.assetIntegrityReport.orphaned.map((asset) => <div key={asset.assetId}><strong>{asset.assetId}</strong><small>{formatByteSize(asset.size)} · {asset.mimeType}{asset.category ? ` · ${asset.category}` : ''}</small></div>)}</div></details>}
        </>}
      </div>}
      {props.workshopIntegrityReport && <div className="asset-integrity-report">
        <h4>创意工坊数据</h4>
        <div className="stat-list"><span>{props.workshopIntegrityReport.packageCount} 个包</span><span>{props.workshopIntegrityReport.bindingCount} 条世界绑定</span><span>{props.workshopIntegrityReport.localStateCount} 份本地 App 状态</span><span>{props.workshopIntegrityReport.referencedAssetCount} 项资产映射</span><span>{props.workshopIntegrityReport.issues.length} 项提示</span></div>
        {props.workshopIntegrityReport.retainedLocalStateCount > 0 && <p className="io-scope">{props.workshopIntegrityReport.retainedLocalStateCount} 份状态属于已卸载包的保留用户资源，不视为损坏，也不会自动删除。</p>}
        {props.workshopIntegrityReport.issues.length === 0 ? <p className="io-scope">工坊包、绑定、依赖和资产映射一致。</p> : <details><summary>工坊完整性提示</summary><div className="integrity-issue-list">{props.workshopIntegrityReport.issues.map((issue, index) => <div key={`${issue.code}:${issue.packageId ?? issue.recordId ?? index}`}><strong>{issue.severity === 'error' ? '错误' : '警告'} · {issue.code}</strong><small>{issue.message}</small></div>)}</div></details>}
      </div>}
      <h3>网络请求</h3>
...
function StorySceneLibraryView(props: { save: SaveFile; storyScenePresets: StoryScenePreset[]; onSavePreset: (source: StoryScenePreset, name: string) => Promise<void>; onUpdatePreset: (preset: StoryScenePreset) => Promise<void>; onDeletePreset: (id: string) => Promise<void>; onCreateDraft: (input: StorySceneDraftInput) => void; onEditDraft: (sceneId: string, input: StorySceneDraftInput) => void; onDeleteDraft: (sceneId: string) => void; onConfirmDraft: (sceneId: string) => void; onAdvanceStage: (sceneId: string) => void; onSetStatus: (sceneId: string, status: 'completed' | 'cancelled') => void; onReadStage: (sceneId: string, stageId: string) => void; onSelectStage: (sceneId: string, stageId: string) => void }) {
  ...
  const submit = () => {
    try {
      const generated = generateStorySceneDraftInput({ id: sceneId.trim() || slug(intent), intent, detailedOutline: outline, participantIds, participantNames: participantIds.map((id) => props.save.world.characters[id]?.name ?? id), preset: selectedPreset });
      props.onCreateDraft(generated);
    } catch (error) { window.alert(error instanceof Error ? error.message : 'StoryScene 草案生成失败。'); }
  };
  return <details className="fold-card story-scene-library" open><summary>多人群像剧情</summary><div className="fold-body"><StoryScenePresetManager presets={presets} onSave={props.onSavePreset} onUpdate={props.onUpdatePreset} onDelete={props.onDeletePreset} /><div className="list-card"><p className="io-scope">StoryScene 是独立的多人群像剧情模式；时间与地点由你在详细大纲中自由描述，不绑定世界时钟或玩家位置，也不会直接推进世界事实。</p><div className="story-scene-create-form"><div className="field-with-action"><label>剧情意图<input value={intent} onChange={(event) => setIntent(event.target.value)} placeholder="例如：寻找失踪的信件" /></label><label>草案 ID<input value={sceneId} onChange={(event) => setSceneId(event.target.value)} placeholder="留空则按意图生成" /></label></div><label>详细大纲（可选）<textarea value={outline} onChange={(event) => setOutline(event.target.value)} placeholder="留空则使用本地预设生成三幕静态内容" /></label><label>本地预设<select value={presetId} onChange={(event) => setPresetId(event.target.value)}>{presets.map((preset) => <option key={preset.id} value={preset.id}>{preset.name}</option>)}</select></label><div><span className="field-label">参与者</span><div className="checkbox-grid">{characters.map((character) => <label className="checkbox-line" key={character.id}><input type="checkbox" checked={participantIds.includes(character.id)} onChange={(event) => toggleParticipant(character.id, event.target.checked)} />{character.name}</label>)}</div></div><button type="button" onClick={submit} disabled={!intent.trim() || participantIds.length === 0}>生成本地草案</button></div></div>{scenes.length === 0 ? <p className="empty">暂无群像剧情。创建并确认 StoryScene 后，会在这里恢复阅读。</p> : scenes.map((scene) => scene.status === 'draft' ? <StorySceneDraftRow key={scene.id} scene={scene} save={props.save} onEdit={props.onEditDraft} onDelete={props.onDeleteDraft} onConfirm={props.onConfirmDraft} /> : <StorySceneReader key={scene.id} scene={scene} onAdvanceStage={() => props.onAdvanceStage(scene.id)} onSetStatus={(status) => props.onSetStatus(scene.id, status)} onReadStage={(stageId) => props.onReadStage(scene.id, stageId)} onSelectStage={(stageId) => props.onSelectStage(scene.id, stageId)} />)}</div></details>;
}
...
function StorySceneDraftRow(props: { scene: SaveFile['world']['storyScenes'][number]; save: SaveFile; onEdit: (sceneId: string, input: StorySceneDraftInput) => void; onDelete: (sceneId: string) => void; onConfirm: (sceneId: string) => void }) {
  ...
  return <div className="story-scene-draft-editor"><div className="field-with-action"><label>标题<input value={title} onChange={(event) => setTitle(event.target.value)} /></label><label>剧情意图<input value={intent} onChange={(event) => setIntent(event.target.value)} /></label></div><label>大纲<textarea value={outline} onChange={(event) => setOutline(event.target.value)} /></label><div className="checkbox-grid">{Object.values(props.save.world.characters).map((character) => <label className="checkbox-line" key={character.id}><input type="checkbox" checked={participantIds.includes(character.id)} onChange={(event) => toggle(character.id, event.target.checked)} />{character.name}</label>)}</div><div className="button-row"><button type="button" onClick={() => { props.onEdit(props.scene.id, { id: props.scene.id, title, intent, outline, participantIds, stages: props.scene.stages, currentStageId: props.scene.currentStageId, source: props.scene.source }); setEditing(false); }}>保存编辑</button><button type="button" className="secondary" onClick={() => setEditing(false)}>取消</button></div></div>;
}
    ...
    {activePanel === 'more' && <section className="terminal-more-panel" aria-label="更多功能">
      ...
      <section className="terminal-appointment-panel" aria-label="远程约定">
        ...
        <div className="terminal-appointment-form">
          ...
          <div className="button-row"><button type="button" onClick={() => { props.onCreateTerminalAppointment(selected.id, appointmentInput()); setAppointmentNote(''); }}>发起约定</button><button type="button" className="secondary" onClick={() => props.onSimulateIncomingAppointment(selected.id, appointmentInput())}>让TA提议</button></div>
        </div>
        {appointmentRequests.length > 0 && <div className="terminal-appointment-list">{appointmentRequests.map((request) => { const appointmentId = `terminal-appointment-${request.id}`; const inCalendar = props.save.world.appointments.some((appointment) => appointment.id === appointmentId); const status = inCalendar ? '已加入日历' : request.status === 'pending' ? request.direction === 'outgoing' ? '已同意' : '待确认' : request.status === 'accepted' ? '已同意' : request.status === 'rejected' ? '已拒绝' : '已撤回'; const canJoinCalendar = request.status === 'accepted' || (request.status === 'pending' && request.direction === 'outgoing'); return <div className="terminal-appointment-row" key={request.id}><span><strong>{request.direction === 'outgoing' ? '我发起' : 'TA发起'} · 第 {request.day} 天 · {props.save.config.calendar.slots.find((slot) => slot.id === request.slotId)?.name ?? request.slotId}</strong><small>{props.save.world.map.nodes[request.nodeId]?.name ?? request.nodeId}{request.note ? ` · ${request.note}` : ''} · {status}</small></span><div className="button-row">{request.status === 'pending' && request.direction === 'incoming' && <><button type="button" onClick={() => props.onResolveTerminalAppointment(request.id, 'accept')}>接受</button><button type="button" className="secondary" onClick={() => props.onResolveTerminalAppointment(request.id, 'reject')}>拒绝</button></>}{canJoinCalendar && !inCalendar && <button type="button" onClick={() => props.onConfirmTerminalAppointment(request.id)}>加入日历</button>}</div></div>; })}</div>}
      </section>
      </>}
...
function LibraryView(props: { appName: string; characters: CharacterCard[]; worldbooks: WorldbookEntry[]; providers: ProviderConfig[]; ttsConfigs: TtsConfig[]; characterBindings: CharacterProviderBinding[]; onCharacterProviderBindingChange: (characterId: string, kind: 'provider' | 'tts', providerId: string) => Promise<void>; presets: Preset[]; presetBundles: PresetBundle[]; selectedPresetBundleId: string; setSelectedPresetBundleId: (value: string) => void; presetBundleName: string; setPresetBundleName: (value: string) => void; onCreatePresetBundle: () => Promise<void>; onRenamePresetBundle: () => Promise<void>; onDeletePresetBundle: (id: string) => Promise<void>; onSetPresetEntryEnabled: (bundleId: string, entryId: string, enabled: boolean) => Promise<void>; onMovePresetEntry: (bundleId: string, entryId: string, direction: -1 | 1) => Promise<void>; save: SaveFile; name: string; setName: (value: string) => void; draftText: string; setDraftText: (value: string) => void; worldbookKeys: string; setWorldbookKeys: (value: string) => void; editing: { kind: ContentKind; id: string } | null; setEditing: (editing: { kind: ContentKind; id: string } | null) => void; addContent: (kind: ContentKind) => Promise<void>; onDelete: (kind: ContentKind, id: string) => Promise<void>; onExport: (kind: ContentKind, value: unknown, name: string) => void; onImport: (kind: ContentKind, file?: File) => Promise<void>; onImportText: (kind: TextImportKind, file?: File) => Promise<void>; onExportSave: () => Promise<void>; onImportSave: (file?: File) => Promise<void>; onExportPresetBundle: () => Promise<void>; onImportPresetBundle: (file?: File) => Promise<void>; includeChatsOnExport: boolean; setIncludeChatsOnExport: (value: boolean) => void; onClearChats: () => Promise<void>; itemName: string; setItemName: (value: string) => void; itemTags: string; setItemTags: (value: string) => void; itemDescription: string; setItemDescription: (value: string) => void; onAddItem: () => void; onAddCharacterToWorld: (id: string) => void; onExportCharacterPackage: (id: string, includeImages: boolean) => Promise<void>; onImportCharacterPackage: (file?: File, addToWorld?: boolean) => Promise<void>; onExportWorldPackage: (includeImages: boolean) => Promise<void>; onImportWorldPackage: (file?: File) => Promise<void>; onExportEventPackage: () => Promise<void>; onImportEventPackage: (file?: File) => Promise<void>; visualCharacterId: string; setVisualCharacterId: (value: string) => void; onImportCharacterVisual: (characterId: string, kind: 'avatar' | 'portrait', source?: File | string) => Promise<void>; onRemoveCharacterVisual: (characterId: string, kind: 'avatar' | 'portrait') => Promise<void>; onUpdateCharacterAccentColor: (characterId: string, color?: string) => void; onOpenImageSettings: () => void; onRequestFriend: (characterId: string, direction: ContactDirection) => void; onResolveFriend: (requestId: string, action: 'accept' | 'reject' | 'revoke') => void; onPromoteNpc: (characterId: string, draft: NpcPromotionDraft) => Promise<boolean>; onExpandNpcPromotionDraft: (characterId: string, draft: NpcPromotionDraft) => Promise<NpcPromotionDraft | undefined>; onSendTerminalText: (characterId: string, text: string, quoteMessageId?: string) => void; onSendTerminalStickerUrl: (characterId: string, url: string, quoteMessageId?: string) => void; onSendTerminalStickerFile: (characterId: string, file?: File, quoteMessageId?: string) => Promise<void>; onSendStickerAsset: (characterId: string, asset: AssetRef, quoteMessageId?: string) => void; stickers: TerminalStickerRecord[]; onImportStickerFile: (file?: File) => Promise<void>; onImportStickerUrl: (url: string) => Promise<void>; onDeleteSticker: (record: TerminalStickerRecord) => Promise<void>; onRejoin: (characterId: string, requirement: string) => void; onEditTerminalMessage: (characterId: string, messageId: string, text: string) => void; onDeleteTerminalMessage: (characterId: string, messageId: string) => void; onGenerateTerminalReply: (characterId: string) => Promise<void>; onSendVoice: (characterId: string, messageId: string, requestId?: string) => Promise<void>; onDownloadVoice: (characterId: string, messageId: string) => Promise<void>; isVoiceAvailable: (characterId: string) => boolean; ttsConfig: TtsConfig; ttsBusy: boolean; onSendPlayerTransfer: (characterId: string, currencyId: string, amount: number) => void; onResolveIncomingTransfer: (requestId: string, action: TransferAction) => void; onCreateTerminalAppointment: (characterId: string, input: TerminalAppointmentInput) => void; onSimulateIncomingAppointment: (characterId: string, input: TerminalAppointmentInput) => void; onResolveTerminalAppointment: (requestId: string, action: TerminalAppointmentAction) => void; onSimulateAppointmentAcceptance: (requestId: string) => void; onConfirmTerminalAppointment: (requestId: string) => void; terminalCall: TerminalCallSession | null; onStartCall: (characterId: string) => void; onSimulateIncomingCall: (characterId: string) => void; onAnswerCall: () => void; onSimulateCallAnswer: () => void; onEndCall: () => void; terminalBusy: boolean; musicPlayer: MusicPlayerController; enabledWorkshopPackages: WorkshopPackageRecord[]; workshopDraftProviderConfigured: boolean; onGenerateWorkshopDraft: (requirement: string, signal?: AbortSignal) => ReturnType<typeof requestWorkshopDraft>; onWorkshopAgentTurn: (input: WorkshopAgentTurnInput, signal?: AbortSignal) => ReturnType<typeof requestWorkshopAgentTurn>; onRunWorkshopActivity: (packageId: string, ruleId: string) => Promise<{ ok: boolean; message: string }>; onRunWorkshopProviderText: (packageId: string, action: WorkshopProviderTextAction, values: Readonly<Record<string, WorkshopLocalValue>>, signal?: AbortSignal) => Promise<{ ok: boolean; message: string; text?: string; resultKey?: string }>; selectedPresetId?: string; setSelectedPresetId?: (value: string) => void }) {
  ...
  if (navigation.activePage === 'workshop') return <SubpageShell eyebrow="终端" title={pageTitle} pageId={navigation.activePage} onBack={navigation.onBack}><WorkshopManager save={props.save} draftProviderConfigured={props.workshopDraftProviderConfigured} onGenerateDraft={props.onGenerateWorkshopDraft} onAgentTurn={props.onWorkshopAgentTurn} /></SubpageShell>;
...
function ContentList(props: { title: string; kind: ContentKind; items: Array<{ id: string; name: string; text: string; keys?: string[] }>; onEdit: (kind: ContentKind, item: { id: string; name: string; text: string; keys?: string[] }) => void; onDelete: (kind: ContentKind, id: string) => Promise<void>; onExport: (kind: ContentKind, value: unknown, name: string) => void; onImport: (kind: ContentKind, file?: File) => Promise<void>; onImportText?: (kind: TextImportKind, file?: File) => Promise<void>; onAction?: (id: string) => void; actionLabel?: string }) {
  return <details className="fold-card"><summary>{props.title}</summary><div className="fold-body"><div className="list-card"><div className="list-heading"><h3>{props.title}</h3><label className="file-button">导入<input type="file" accept=".json" onChange={(event) => void props.onImport(props.kind, event.target.files?.[0])} /></label></div>{props.items.length === 0 ? <p className="empty">暂无内容</p> : props.items.map((item) => <div className="list-row" key={item.id}><span>{item.name}{props.kind === 'worldbook' && <small>{item.keys?.length ? `关键词：${item.keys.join('、')}` : '常驻触发'}</small>}</span><span className="button-row">{props.onAction && <button className="secondary" onClick={() => props.onAction?.(item.id)}>{props.actionLabel ?? '应用'}</button>}<button onClick={() => props.onEdit(props.kind, item)}>编辑</button><button onClick={() => props.onExport(props.kind, item, item.name)}>导出</button><button onClick={() => void props.onDelete(props.kind, item.id)}>删除</button></span></div>)}</div></div></details>;
}
