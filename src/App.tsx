...
import { confirmStoryScene, createBuiltinStoryScenePresets, createStorySceneDraft, formatChatArchive, formatEventHistoryArchive, generateStorySceneDraftInput, readStorySceneStage, selectStorySceneReadingStage, type StorySceneDraftInput } from './core/story';
...
const defaultSave: SaveFile = SaveFileSchema.parse({
  ...
  world: {
    ...
    relations: {}, characters: {}, npcs: {}, npcTemplates: {}, encounterLog: [], topicTrees: {}, usedTopics: {}, appointments: [], eventDefs: {}, director: { scheduled: [], lastFiredDay: {}, tension: 0, tensionOffset: 0, tensionUpdatedDay: 1 }, eventHistory: [], chapters: [], milestones: [], storyScenes: [], collection: [],
...
export function App() {
  ...
  return <div className="app-shell">
    ...
    <main className={`screen ${tab === 'chat' ? 'chat-screen-host' : ''} ${tab === 'map' ? 'map-screen-host' : ''}`}>
      ...
      {tab === 'library' && <StorySceneLibraryView save={save} onCreateDraft={createStorySceneDraftFromInput} onConfirmDraft={confirmStorySceneDraft} onReadStage={(sceneId, stageId) => updateStorySceneReading(sceneId, stageId, 'read')} onSelectStage={(sceneId, stageId) => updateStorySceneReading(sceneId, stageId, 'select')} />}
...
function StorySceneLibraryView(props: { save: SaveFile; onCreateDraft: (input: StorySceneDraftInput) => void; onConfirmDraft: (sceneId: string) => void; onReadStage: (sceneId: string, stageId: string) => void; onSelectStage: (sceneId: string, stageId: string) => void }) {
  const scenes = props.save.world.storyScenes ?? [];
  const presets = createBuiltinStoryScenePresets();
  const characters = Object.values(props.save.world.characters);
  ...
  return <details className="fold-card story-scene-library"><summary>多人群像剧情</summary><div className="fold-body"><div className="list-card"><p className="io-scope">StoryScene 是独立的多人群像剧情模式；以下表单只生成本地可编辑草案，不调用 API，也不会直接推进世界事实。</p><div className="story-scene-create-form"><div className="field-with-action"><label>剧情意图<input value={intent} onChange={(event) => setIntent(event.target.value)} placeholder="例如：寻找失踪的信件" /></label><label>草案 ID<input value={sceneId} onChange={(event) => setSceneId(event.target.value)} placeholder="留空则按意图生成" /></label></div><label>详细大纲（可选）<textarea value={outline} onChange={(event) => setOutline(event.target.value)} placeholder="留空则使用本地预设生成三幕静态内容" /></label><div className="field-with-action"><label>地点<select value={nodeId} onChange={(event) => setNodeId(event.target.value)}>{nodes.map((node) => <option key={node.id} value={node.id}>{node.name}</option>)}</select></label><label>开始日期<input type="number" min={props.save.world.clock.day} value={startDay} onChange={(event) => setStartDay(event.target.value)} /></label><label>开始时段<select value={startSlotId} onChange={(event) => setStartSlotId(event.target.value)}>{props.save.config.calendar.slots.map((slot) => <option key={slot.id} value={slot.id}>{slot.label}</option>)}</select></label><label>本地预设<select value={presetId} onChange={(event) => setPresetId(event.target.value)}>{presets.map((preset) => <option key={preset.id} value={preset.id}>{preset.name}</option>)}</select></label></div><div><span className="field-label">参与者</span><div className="checkbox-grid">{characters.map((character) => <label className="checkbox-line" key={character.id}><input type="checkbox" checked={participantIds.includes(character.id)} onChange={(event) => toggleParticipant(character.id, event.target.checked)} />{character.name}</label>)}</div></div><button type="button" onClick={submit} disabled={!intent.trim() || participantIds.length === 0 || !nodeId}>生成本地草案</button></div></div>{scenes.length === 0 ? <p className="empty">暂无群像剧情。创建并确认 StoryScene 后，会在这里恢复阅读。</p> : scenes.map((scene) => scene.status === 'draft' ? <div className="story-scene-draft-row" key={scene.id}><div><strong>{scene.title}</strong><small>草案 · 尚未确认启动 · {scene.participantIds.length} 位参与者</small></div><button type="button" onClick={() => props.onConfirmDraft(scene.id)}>确认启动</button></div> : <StorySceneReader key={scene.id} scene={scene} onReadStage={(stageId) => props.onReadStage(scene.id, stageId)} onSelectStage={(stageId) => props.onSelectStage(scene.id, stageId)} />)}</div></details>;
}
