...
export function App() {
  ...
  async function importCharacterFaceReference(characterId: string, file?: File): Promise<void> {
    if (!file) return;
    ...
    try {
      ...
      await saveAsset({ id: assetId, blob: image.blob, mimeType: image.mimeType, width: image.width, height: image.height, createdAt: now() });
      const existing = imageVisualConfigs.find((item) => item.saveId === saveRef.current.meta.id && item.characterId === characterId);
      const previousAssetId = existing?.referenceImage?.assetId;
  ...
  async function importUserFaceReference(file?: File): Promise<void> {
    if (!file) return;
    let assetId: string | undefined;
    try {
      const identity = currentImageIdentity(saveRef.current.world.player.personaId);
  ...
  return <div className="library-subpage-content"><section className="library-legacy-content">
    ...
    <div className="provider-card">
      ...
      {worldCharacters.length ? <>
        ...
        <p className="io-scope">{selectedVisual?.referenceImage ? `已保存本地参考图：${selectedVisual.referenceImage.assetId}` : '尚未上传角色锁脸参考图。'} {selectedVisual?.lockFaceEnabled && draft.referenceMode !== 'openai-edits' ? '当前 Provider 未启用 edits，生成时只使用固定外貌提示词，不会发送参考图。' : selectedVisual?.lockFaceEnabled ? '生成时会将该参考图发送到你配置的 edits 端点。' : ''}</p>
    ...
    <div className="provider-card">
      ...
      <p className="io-scope">{userVisual?.referenceImage ? `已保存本地参考图：${userVisual.referenceImage.assetId}` : '尚未上传用户锁脸参考图。'} {userVisual?.lockFaceEnabled && draft.referenceMode !== 'openai-edits' ? '当前 Provider 未启用 edits；未来涉及用户的生成只会使用固定外貌描述。' : '本切片不会自动生成或替换玩家头像。'}</p>
...
function SettingsView(props: {
  ...
  onImportCharacterFaceReference: (characterId: string, file?: File) => Promise<void>;
