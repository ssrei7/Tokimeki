...
function MapView({ save, worldbooks, activeEncounter, onEncounterOutcome, onContinueEncounter, onMove, onOpenChat, onImportBackground, onImportSceneBackground, onRemoveSceneBackground, onToggleMode, onCreateNode, onEditNode, onDeleteNode, onSuggestNode, onGenerateMap, onExpandMap, mapGenerating }: { save: SaveFile; worldbooks: WorldbookEntry[]; activeEncounter: ActiveEncounter | null; onEncounterOutcome: (outcome: 'continued' | 'urgent_leave') => void; onContinueEncounter: () => void; onMove: (nodeId: string) => void; onOpenChat: () => void; onImportBackground: (file?: File) => Promise<void>; onImportSceneBackground: (nodeId: string, file?: File) => Promise<void>; onRemoveSceneBackground: (nodeId: string) => Promise<void>; onToggleMode: () => void; onCreateNode: (input: CreateMapNodeInput) => boolean; onEditNode: (nodeId: string, input: UpdateMapNodeInput) => boolean; onDeleteNode: (nodeId: string) => boolean; onSuggestNode: (input: { requirements: string; regionName: string; anchorName: string }) => Promise<{ name: string; description: string } | null>; onGenerateMap: (requirements?: string) => Promise<void>; onExpandMap: (anchorNodeId: string, count: number, requirements?: string) => Promise<void>; mapGenerating: boolean }) {
  ...
  const selectMapNode = (nodeId: string) => { if (!map.nodes[nodeId]?.discovered) return; setSelectedMapNodeId(nodeId); toolSheetRef.current?.removeAttribute('open'); detailSheetRef.current?.setAttribute('open', ''); };
  ...
  const endSheetDrag = (event: PointerEvent<HTMLElement>) => { if (sheetDragRef.current.pointerId !== event.pointerId) return; const deltaY = event.clientY - sheetDragRef.current.startY; const moved = sheetDragRef.current.moved; sheetDragRef.current.pointerId = -1; if (!moved) return; event.preventDefault(); const sheet = event.currentTarget.closest('details') as HTMLDetailsElement | null; if (deltaY < -24) sheet?.setAttribute('open', ''); else if (deltaY > 24) sheet?.removeAttribute('open'); };
  const cancelSheetClick = (event: MouseEvent<HTMLElement>) => { if (!sheetDragRef.current.moved) return; event.preventDefault(); event.stopPropagation(); sheetDragRef.current.moved = false; };
  return <section className="map-screen">
    ...
    <details ref={toolSheetRef} className="map-menu map-bottom-sheet map-tool-sheet">
      <summary onPointerDown={beginSheetDrag} onPointerMove={moveSheetDrag} onPointerUp={endSheetDrag} onPointerCancel={endSheetDrag} onClick={cancelSheetClick}><span>地图工具{editorMode ? ' · 编辑中' : ''}</span><span>向上展开</span></summary>
    ...
    {selectedMapNodeId && <details ref={detailSheetRef} open onToggle={() => { if (!detailSheetRef.current?.open) setSelectedMapNodeId(null); }} className="map-menu map-bottom-sheet map-detail-sheet">
      <summary onPointerDown={beginSheetDrag} onPointerMove={moveSheetDrag} onPointerUp={endSheetDrag} onPointerCancel={endSheetDrag} onClick={cancelSheetClick}><span>{selectedMapNode?.name ?? '地点详情'}</span><span>向下收起</span></summary>
