...
export function App() {
  ...
  useEffect(() => {
    if (tab === 'settings' && settingsPage === 'privacy') { void refreshVoiceCacheStats(); void refreshImageAssetStats(); }
  }, [settingsPage, tab]);
  useEffect(() => {
  ...
  async function clearUnusedImageAssets(): Promise<void> {
    try {
      ...
      const cleanedCharacterConfigs = characterConfigs.map((config) => {
        if (!config.referenceImage || available.has(config.referenceImage.assetId)) return config;
