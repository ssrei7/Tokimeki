...
function GraphMapPresence({ people, visuals, avatarUrls, x, y }: { people: ReturnType<typeof whoIsWhere>; visuals: Record<string, MapPresenceVisual>; avatarUrls: Record<string, string>; x: number; y: number }) {
  ...
  return <g className="map-presence-stack" transform={`translate(${x + 29} ${y - 38})`} aria-label={`这里有${people.map((person) => person.name).join('、')}`}>{people.slice(0, 3).map((person, index) => { const visual = visuals[person.id]; const avatarUrl = avatarUrls[person.id]; const avatarX = index * 28; return <g key={person.id} transform={`translate(${avatarX} 0)`}><title>{person.name}</title><circle className="map-presence-avatar-ring" cx="0" cy="0" r="16" /><circle cx="0" cy="0" r="14" fill={visual?.accentColor ?? '#667085'} />{avatarUrl ? <image className="map-presence-avatar-image" href={avatarUrl} x="-14" y="-14" width="28" height="28" preserveAspectRatio="xMidYMid slice" /> : <text className="map-presence-avatar-initial" x="0" y="5" textAnchor="middle">{visual?.initial ?? '?'}</text>}</g>; })}{people.length > 3 && <text className="map-presence-more" x="84" y="5">+{people.length - 3}</text>}</g>;
}
