...
import { EventPackageSchema, PresetBundleSchema, PresetSchema, type CharacterCard, type ChatMessage, type ChatRecord, type EventPackage, type Persona, type Preset, type PresetBundle, type WorldbookEntry } from './data/content';
...
import { exportEventPackage, exportPresetBundle, exportSaveZip, importEventPackage, importPresetBundle, importSaveZip } from './data/io/zip';
...
export function App() {
  ...
  function exportEventHistory(): void {
    ...
    anchor.href = url; anchor.download = 'tokimeki-event-history.json'; anchor.click(); URL.revokeObjectURL(url);
    setFeedback({ tone: 'success', text: `已导出 ${history.length} 条事件回顾。` });
  }
  ...
  async function exportEventPackageFile(): Promise<void> {
    const events = Object.values(saveRef.current.world.eventDefs ?? {});
    if (events.length === 0) { setFeedback({ tone: 'info', text: '当前世界没有可导出的事件定义。' }); return; }
    const pack: EventPackage = EventPackageSchema.parse({ id: 'world-events', name: '当前世界事件包', events, updatedAt: now() });
    const blob = await exportEventPackage(pack);
    const url = URL.createObjectURL(blob); const anchor = document.createElement('a');
    anchor.href = url; anchor.download = 'tokimeki-events.zip'; anchor.click(); URL.revokeObjectURL(url);
    setFeedback({ tone: 'success', text: `已导出事件包“${pack.name}”（${events.length} 个事件）。` });
  }

  async function importEventPackageFile(file?: File): Promise<void> {
    if (!file) return;
    try {
      const imported = await importEventPackage(file);
      const next = structuredClone(saveRef.current);
      const result = installEventDefs(next.world, imported.pack.events);
      commitSave(next);
      setFeedback({ tone: 'success', text: `已导入事件包“${imported.pack.name}”：新增 ${result.added} 个，替换 ${result.replaced} 个。` });
    } catch (error) { setFeedback({ tone: 'error', text: errorMessage(error, '事件包导入失败') }); }
  }
...
function DayView(props: { save: SaveFile; snapshots: SaveSnapshot[]; morningBriefs: SaveFile['world']['morningBriefs']; morningUpdates: SaveFile['world']['morningUpdates']; morningStyle: SaveFile['config']['morningStyle']; summarizingDay: number | null; onAction: (kind: string) => void; onSleep: () => void; onRestoreSnapshot: (id: string) => Promise<void>; onSaveDiary: (day: number, text: string) => void; onPresetChange: (preset: SaveFile['config']['calendar']['preset']) => void; onRevealEvent: (scheduledId: string) => void; onResolveEventChoice: (historyId: string, choiceId: string) => void; onExportEventHistory: () => void; onDeleteEventHistory: (id?: string) => void; onMove: (nodeId: string) => void }) {
  ...
  return <section>
    ...
    <div className="day-card"><label>每日节奏<select value={calendar.preset} disabled={used > 0} onChange={(event) => props.onPresetChange(event.target.value as SaveFile['config']['calendar']['preset'])}><option value="leisure">悠闲 · 6 时段</option><option value="standard">标准 · 4 时段</option><option value="tight">紧凑 · 3 时段</option><option value="sandbox">沙盒 · 不消耗</option></select></label><p className="io-scope">行动只修改本地确定性状态，不调用 API。节奏仅能在当天尚未行动时切换。</p><div className="day-actions">{actionButtons.map((action) => <button key={action.kind} onClick={() => props.onAction(action.kind)} disabled={!calendar.unlimitedSlots && action.cost > remaining}>{action.label}<small>{calendar.unlimitedSlots ? '不消耗' : `${action.cost} 时段`}</small></button>)}<button className="secondary" onClick={props.onSleep}>推进世界<small>结算今天 · 进入下一天</small></button></div></div>
    <div className="morning-brief-card"><div className="list-heading"><div><span className="eyebrow">{morningPresentation.eyebrow}</span><h3>{morningPresentation.title}</h3></div><span className="io-scope">最多每日一次合并更新</span></div>{todayUpdate && <div className="morning-world-meta"><span>天气：{todayUpdate.weather.label}</span>{todayUpdate.worldNote && <span>{todayUpdate.worldNote}</span>}</div>}{todayBriefs.length ? <div className="morning-brief-list">{todayBriefs.map((entry) => { const destination = resolveMorningAdDestination(entry, props.save.world); const isJobAd = entry.category === 'ad' && entry.entryKind === 'job'; const isHousingAd = entry.category === 'ad' && entry.entryKind === 'housing'; const isShopTransferAd = entry.category === 'ad' && entry.entryKind === 'shop_transfer'; return <article key={entry.id} className={`morning-brief-entry morning-${entry.category}`}><span className="eyebrow">{entry.category}</span><strong>{entry.title}</strong><p>{entry.body}</p>{(destination || isJobAd || isHousingAd || isShopTransferAd) && <div className="morning-entry-actions">{destination && <button className="secondary" onClick={() => props.onMove(destination.id)}>前往{destination.name}</button>}{isJobAd && <button onClick={() => openAd(entry)}>查看招聘</button>}{isHousingAd && <button onClick={() => openAd(entry)}>查看住房</button>}{isShopTransferAd && <button onClick={() => openAd(entry)}>查看转让</button>}</div>}</article>; })}</div> : <p className="empty">{morningPresentation.empty}</p>}{activeAd?.entryKind === 'job' && <div className="morning-ad-entry"><div className="list-heading"><h3>招聘入口</h3><button className="secondary" onClick={() => setActiveAd(null)}>关闭</button></div><p>{activeAd.body}</p><button onClick={() => { props.onAction('work'); setActiveAd(null); }} disabled={!calendar.unlimitedSlots && (props.save.config.actionCosts.work?.slotCost ?? 0) > remaining}>接受招聘并开始工作<small>{calendar.unlimitedSlots ? '不消耗时段' : `${props.save.config.actionCosts.work?.slotCost ?? 0} 时段`}</small></button></div>}{activeAd?.entryKind === 'housing' && <div className="morning-ad-entry"><div className="list-heading"><h3>住房入口</h3><button className="secondary" onClick={() => setActiveAd(null)}>关闭</button></div><p>{activeAd.body}</p><div className="morning-housing-options"><button className={housingMode === 'rent' ? 'selected' : ''} onClick={() => setHousingMode('rent')}>查看租房方案</button><button className={housingMode === 'buy' ? 'selected' : ''} onClick={() => setHousingMode('buy')}>查看买房方案</button></div>{housingMode && <div className="morning-housing-detail"><strong>{housingMode === 'rent' ? '租房方案' : '买房方案'}</strong><p>{housingMode === 'rent' ? '先查看可租住所与入住条件。正式租金、合同和入住事实将在货币与住房系统接入后落库。' : '先查看可购买住所与入住条件。正式价格、产权和购买事实将在货币与住房系统接入后落库。'}</p></div>}</div>}{activeAd?.entryKind === 'shop_transfer' && <div className="morning-ad-entry"><div className="list-heading"><h3>店铺转让入口</h3><button className="secondary" onClick={() => setActiveAd(null)}>关闭</button></div><p>{activeAd.body}</p><button onClick={() => setShopTransferOpen(true)}>查看转让方案</button>{shopTransferOpen && <div className="morning-housing-detail"><strong>转让方案</strong><p>这里会承接店铺位置、营业条件和转让说明。正式经营权、价格与资金结算将在货币与住房系统接入后落库。</p></div>}</div>}</div>
...
function EventPackageView(props: { save: SaveFile; onExportEventPackage: () => Promise<void>; onImportEventPackage: (file?: File) => Promise<void> }) {
  const events = Object.values(props.save.world.eventDefs ?? {});
  return <div className="list-card event-package-card"><div className="list-heading"><div><h3>事件包</h3><p className="io-scope">导入或导出当前世界的纯本地静态事件定义；不会调用 API。导入后按事件 ID 替换同名事件。</p></div><div className="button-row"><button className="secondary" disabled={events.length === 0} onClick={() => void props.onExportEventPackage()}>导出当前事件包</button><label className="file-button">导入事件包<input type="file" accept=".zip" onChange={(event) => void props.onImportEventPackage(event.target.files?.[0])} /></label></div></div><p className="io-scope">当前世界共有 {events.length} 个事件定义。</p>{events.length === 0 ? <p className="empty">暂无事件定义。导入一个事件包后，事件会参与确定性排程。</p> : <div className="tag-row">{events.slice(0, 12).map((event) => <span className="tag" key={event.id}>{event.title}</span>)}{events.length > 12 && <span className="tag">另有 {events.length - 12} 个</span>}</div>}</div>;
}
