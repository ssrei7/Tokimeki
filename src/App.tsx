...
import { formatChatArchive, formatEventHistoryArchive, readStorySceneStage, selectStorySceneReadingStage } from './core/story';
...
export function App() {
  ...
  return <div className="app-shell">
    ...
    <main className={`screen ${tab === 'chat' ? 'chat-screen-host' : ''} ${tab === 'map' ? 'map-screen-host' : ''}`}>
      ...
      {tab === 'library' && <StorySceneLibraryView save={save} onReadStage={(sceneId, stageId) => updateStorySceneReading(sceneId, stageId, 'read')} onSelectStage={(sceneId, stageId) => updateStorySceneReading(sceneId, stageId, 'select')} />}
...
function StorySceneLibraryView(props: { save: SaveFile; onReadStage: (sceneId: string, stageId: string) => void; onSelectStage: (sceneId: string, stageId: string) => void }) {
  const scenes = props.save.world.storyScenes ?? [];
  return <details className="fold-card story-scene-library"><summary>多人群像剧情</summary><div className="fold-body"><div className="list-card"><p className="io-scope">StoryScene 是独立的多人群像剧情模式；这里先折叠展示，未来可替换为统一图标入口。</p>{scenes.length === 0 ? <p className="empty">暂无群像剧情。创建并确认 StoryScene 后，会在这里恢复阅读。</p> : scenes.map((scene) => scene.status === 'draft' ? <div className="story-scene-draft-row" key={scene.id}><strong>{scene.title}</strong><small>草案 · 尚未确认启动</small></div> : <StorySceneReader key={scene.id} scene={scene} onReadStage={(stageId) => props.onReadStage(scene.id, stageId)} onSelectStage={(stageId) => props.onSelectStage(scene.id, stageId)} />)}</div></div></details>;
}
