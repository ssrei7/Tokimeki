  ...
  return <section className="map-screen">
    ...
    <div ref={mapCanvasRef} className="map-canvas" onClick={handleMapCanvasClick} onWheel={mapWheel} onPointerDown={beginPan} onPointerMove={movePan} onPointerUp={endPan} onPointerCancel={endPan}>
      {map.view.mode === 'graph' ? <svg ref={graphSurfaceRef} className={`map-svg ${editorMode ? 'editing' : ''}`} style={{ transform: `translate(${offset.x}px, ${offset.y}px) scale(${zoom})`, transformOrigin: '0 0' }} viewBox={`0 0 ${map.view.size.w} ${map.view.size.h}`} role="img" aria-label="世界地图">
        ...
        <g className="map-nodes">{nodes.map((node) => { const isCurrent = node.id === save.world.player.nodeId; const canSelect = node.discovered && !isCurrent; return <g key={node.id} className={`map-node ${node.discovered ? 'discovered' : 'undiscovered'} ${isCurrent ? 'current' : ''}`} role={editorMode || canSelect ? 'button' : undefined} tabIndex={editorMode || canSelect ? 0 : undefined} onClick={(event) => { event.stopPropagation(); if (editorMode) beginEditNode(node.id); else if (canSelect && !dragRef.current.moved) onMove(node.id); }} onKeyDown={(event) => { if ((editorMode || canSelect) && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); if (editorMode) beginEditNode(node.id); else onMove(node.id); } }}><circle cx={node.pos.x} cy={node.pos.y} r={isCurrent ? 56 : 46} /><text x={node.pos.x} y={node.pos.y + 92} textAnchor="middle">{node.discovered ? node.name : '未发现地点'}</text>{isCurrent && <text className="map-node-marker" x={node.pos.x} y={node.pos.y + 12} textAnchor="middle">你</text>}<GraphMapPresence people={mapPresence.byNode[node.id] ?? []} visuals={mapPresence.visuals} avatarUrls={mapAvatarUrls} x={node.pos.x} y={node.pos.y} />{encounterTraces[node.id]?.[0] && <text className="map-node-trace" x={node.pos.x} y={node.pos.y + 120} textAnchor="middle">{encounterTraceLabel(encounterTraces[node.id][0])}</text>}</g>; })}</g>
    ...
    <details className="map-menu map-bottom-sheet">
      <summary><span>地图工具{editorMode ? ' · 编辑中' : ''}</span><span>向上展开</span></summary>
...
function GraphMapPresence({ people, visuals, avatarUrls, x, y }: { people: ReturnType<typeof whoIsWhere>; visuals: Record<string, MapPresenceVisual>; avatarUrls: Record<string, string>; x: number; y: number }) {
  ...
  return <g className="map-presence-stack" transform={`translate(${x + 116} ${y - 152})`} aria-label={`这里有${people.map((person) => person.name).join('、')}`}>{people.slice(0, 3).map((person, index) => { const visual = visuals[person.id]; const avatarUrl = avatarUrls[person.id]; const avatarX = index * 112; return <g key={person.id} transform={`translate(${avatarX} 0)`}><title>{person.name}</title><circle className="map-presence-avatar-ring" cx="0" cy="0" r="64" /><circle cx="0" cy="0" r="56" fill={visual?.accentColor ?? '#667085'} />{avatarUrl ? <image className="map-presence-avatar-image" href={avatarUrl} x="-56" y="-56" width="112" height="112" preserveAspectRatio="xMidYMid slice" /> : <text className="map-presence-avatar-initial" x="0" y="20" textAnchor="middle">{visual?.initial ?? '?'}</text>}</g>; })}{people.length > 3 && <text className="map-presence-more" x="336" y="20">+{people.length - 3}</text>}</g>;
}
