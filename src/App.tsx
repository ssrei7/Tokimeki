...
export function App() {
  ...
  async function downloadGlobalBackup(includeSecrets: boolean): Promise<void> {
    try {
      ...
      const localStorage: Record<string, string> = {}; for (let index = 0; index < window.localStorage.length; index += 1) { const key = window.localStorage.key(index); if (key?.startsWith('tokimeki.')) { const value = window.localStorage.getItem(key); if (value !== null) localStorage[key] = value; } }
  ...
  async function restoreGlobalBackup(backup: ImportedGlobalBackup, selection: GlobalBackupRestoreSelection = { world: true, content: true, providers: true, assets: true, preferences: true }): Promise<void> {
    ...
    const previous = {
      ...
      preferences: selection.preferences ? Object.fromEntries(Object.keys(backup.data.localStorage).map((key) => [key, window.localStorage.getItem(key)])) : {},
    };
    try {
      ...
      try {
        ...
        if (selection.preferences) { for (const [key, value] of Object.entries(previous.preferences)) { if (value === null || value === undefined) window.localStorage.removeItem(key); else window.localStorage.setItem(key, value); } }
