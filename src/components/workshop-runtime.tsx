...
function isRunnableAction(record: WorkshopPackageRecord, action: WorkshopAction): boolean {
...
function blockedActionLabel(action: WorkshopAction): string {
  if (action.type === 'submit-op') return '状态动作将在确定性活动切片开放';
...
export function WorkshopPageRenderer({ record, save, pageId, values, assetUrls = {}, onPageChange, onValueChange }: { record: WorkshopPackageRecord; save: SaveFile; pageId: string; values: Record<string, WorkshopLocalValue>; assetUrls?: Readonly<Record<string, string>>; onPageChange: (pageId: string) => void; onValueChange: (key: string, value: WorkshopLocalValue) => void }) {
  ...
  const runAction = (action: WorkshopAction) => {
    if (!isRunnableAction(record, action)) return;
  ...
  const renderComponent = (component: WorkshopComponent, index: number): ReactNode => {
    ...
    if (component.kind === 'button') return <button type="button" key={key} disabled={!isRunnableAction(record, component.action)} title={!isRunnableAction(record, component.action) ? blockedActionLabel(component.action) : undefined} onClick={() => runAction(component.action)}>{component.label}</button>;
    ...
    if (component.kind === 'confirm') return <button type="button" key={key} disabled={!isRunnableAction(record, component.action)} title={!isRunnableAction(record, component.action) ? blockedActionLabel(component.action) : undefined} onClick={() => { if (window.confirm(component.message)) runAction(component.action); }}>{component.label}</button>;
    return null;
  };
...
export function WorkshopRuntimeView({ record, save }: { record: WorkshopPackageRecord; save: SaveFile }) {
  ...
  return <div className="library-subpage-content workshop-runtime" data-workshop-package={record.id}>
    ...
    <section className="workshop-content">{loaded ? <WorkshopPageRenderer record={record} save={save} pageId={pageId} values={values} onPageChange={setPageId} onValueChange={(key, value) => setValues((current) => ({ ...current, [key]: value }))} /> : <p className="empty">正在读取本地 App 状态…</p>}</section>
  </div>;
}
