import { CloudDownload, Pause, Play, Plus, Repeat1, Shuffle, SkipBack, SkipForward, Trash2, X } from 'lucide-react';
...
export function MusicApp({ player, onNotify }: { player: MusicPlayerController; onNotify?: (message: string, tone?: 'success' | 'error') => void }) {
  ...
  const cacheTrack = async (trackId: string) => {
    const result = await player.cacheTrackOffline(trackId);
    notify(result.ok ? '外链音频已缓存，可离线播放。' : result.warning ?? '离线缓存失败。', result.ok ? 'success' : 'error');
  };
  const removeCache = async (trackId: string) => {
    const result = await player.removeTrackOfflineCache(trackId);
    notify(result.ok ? '离线缓存已移除，曲目将继续使用原外链。' : result.warning ?? '缓存移除失败。', result.ok ? 'success' : 'error');
  };
  ...
  return <div className="music-app" data-testid="music-app">
    ...
    <details className="surface-card music-list-card" open={playlistOpen} onToggle={(event) => setPlaylistOpen(event.currentTarget.open)}><summary className="list-heading"><div><h3>播放列表</h3><span className="io-scope">{player.state.tracks.length} 首 · {playlistOpen ? '收起' : '展开'}</span></div><span className="io-scope">外链 / 本地</span></summary>{player.state.tracks.length === 0 ? <p className="empty">还没有曲目，请添加 http(s) 音频链接或导入本地文件。</p> : <div className="music-track-list">{player.state.tracks.map((track, index) => { const playing = player.isPlaying && track.id === player.state.currentTrackId; const cacheBusy = player.cacheBusyTrackId === track.id; return <MusicTrackRow key={track.id} track={track} index={index} active={track.id === player.state.currentTrackId} playing={playing} cacheBusy={cacheBusy} cacheLocked={Boolean(player.cacheBusyTrackId)} onSelect={() => player.selectTrack(track.id)} onPlay={() => playing ? player.pause() : player.playTrack(track.id)} onCache={() => void cacheTrack(track.id)} onRemoveCache={() => void removeCache(track.id)} onEdit={() => setEditingId(track.id)} onDelete={() => { const needsConfirm = !track.url && track.asset; if (needsConfirm && !window.confirm(`删除本地曲目“${track.title}”并回收音频文件？`)) return; void player.removeTrack(track.id); if (editingId === track.id) clearForm(); }} onMoveUp={() => player.moveTrack(track.id, -1)} onMoveDown={() => player.moveTrack(track.id, 1)} />; })}</div>}</details>
...
function MusicTrackRow({ track, index, active, playing, cacheBusy, cacheLocked, onSelect, onPlay, onCache, onRemoveCache, onEdit, onDelete, onMoveUp, onMoveDown }: { track: MusicTrack; index: number; active: boolean; playing: boolean; cacheBusy: boolean; cacheLocked: boolean; onSelect: () => void; onPlay: () => void; onCache: () => void; onRemoveCache: () => void; onEdit: () => void; onDelete: () => void; onMoveUp: () => void; onMoveDown: () => void }) {
  const sourceLabel = track.url ? track.asset ? '外链 · 已缓存' : '外链' : '本地导入';
  return <div className={`music-track-row ${active ? 'active' : ''}`}><button type="button" className="music-track-main" onClick={onSelect}><span className="music-track-index">{playing ? '▶' : index + 1}</span><span><strong>{track.title}</strong><small>{track.artist || '未填写艺术家'} · {sourceLabel}</small></span></button><div className="button-row music-track-actions"><button type="button" className="icon-button" aria-label={playing ? '暂停曲目' : '播放曲目'} onClick={onPlay}>{playing ? <Pause aria-hidden="true" /> : <Play aria-hidden="true" />}</button>{track.url && !track.asset && <button type="button" className="text-button" disabled={cacheLocked} onClick={onCache}><CloudDownload aria-hidden="true" />{cacheBusy ? '缓存中…' : '缓存离线'}</button>}{track.url && track.asset && <button type="button" className="text-button" disabled={cacheLocked} onClick={onRemoveCache}>移除缓存</button>}<button type="button" className="text-button" onClick={onMoveUp} disabled={index === 0}>上移</button><button type="button" className="text-button" onClick={onMoveDown}>下移</button><button type="button" className="text-button" onClick={onEdit}>编辑</button><button type="button" className="icon-button danger-icon" aria-label={`删除${track.title}`} onClick={onDelete}><Trash2 aria-hidden="true" /></button></div></div>;
}
