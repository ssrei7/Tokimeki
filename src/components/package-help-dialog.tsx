...
function WorldPackageGuide() {
  return <div className="package-help-content">
    ...
    <section><h3>推荐制作流程</h3><ol><li>先在一个专门用于制作的世界中完成地图、地点、角色、NPC、物品和事件定义。</li><li>检查稳定 ID。地点、角色、物品和事件之间的引用必须使用实际 ID；发布后尽量不要随意改 ID。</li><li>配置头像、立绘和场景背景。需要让接收者离线获得图片时，导出前勾选“含本地图片”。外链图片只保留 URL。</li><li>点击“导出当前世界包”，得到 zip 文件。在另一个测试世界中导入一次，检查地图连接、角色、事件和图片。</li><li>确认无误后再分享 zip。建议同时写明适用的 Tokimeki 版本和包内容简介。</li></ol></section>
...
function EventPackageGuide() {
  return <div className="package-help-content">
    ...
    <section><h3>推荐制作流程</h3><ol><li>如果当前世界已有事件，先点击“导出当前事件包”，把导出的 zip 当作模板。</li><li>完整可视化事件编辑器尚未提供；高级作者可以解压副本，编辑 <code>events.json</code>，再把文件按原目录结构重新压成 zip。</li><li>为每个事件设置稳定且不重复的 <code>id</code>、标题、触发条件，以及 <code>content</code>、<code>prompt</code> 或 <code>choices</code> 中至少一种内容。</li><li>重新导入测试包。应用会检查 schema、地点、角色、关系阶段和安全条件表达式；错误会阻止导入，警告需要用户确认。</li><li>在测试世界走到对应地点和时段，带上要求的角色，确认事件能够进入候选并正常触发。</li></ol></section>
