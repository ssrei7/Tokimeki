...
describe('morning lead hook pool', () => {
  it('keeps untriggered leads available and matches location plus optional slot', () => {
    ...
    const added = syncLeadHooks(save.world, [{ id: 'morning-2-1', day: 2, category: 'lead', title: '码头动静', body: '去码头看看。', eventText: '有人留下了一张新的告示。', nodeId: 'docks', slotId: 'noon', charIds: [], source: 'local' }]);
    expect(added).toBe(1);
    ...
    expect(save.world.map.nodes.docks.memories.at(-1)?.text).toContain('有人留下了一张新的告示');
